...
"""
        ...
        for concept, columns in duplicated_groups.items():
            ...
            for col in columns:
                status = "✅ Primária (com dados)" if col['is_primary'] and col['count'] > 0 else "❌ Legado (vazia/poucos dados)"
                content += f"| {col['name']} | {col['type']} | {col['count']} | {status} |\n"
            content += "\n"
...
2. **Limpeza de Dados:** Corrigir {sanity.get('summary', {}).get('missing_numero_ssa', 0)} SSAs sem número
