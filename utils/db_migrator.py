"""Ferramentas de migração segura do banco de dados SSA.

Fluxo principal:
1. Cria backup do banco original.
2. Analisa estrutura e detecta colunas duplicadas.
3. Gera novo banco com schema otimizado.
4. Consolida dados de colunas duplicadas seguindo prioridades.
5. Verifica integridade e gera relatório de migração.
"""
...
class SafeDatabaseMigrator:
    ...
    def execute_full_migration(self, schema_file: str = "config/schema_optimized.sql") -> Dict[str, Any]:
        """Executa migração completa do banco.

        Etapas: backup, análise, criação novo banco, consolidação, verificação integridade, índices.
        """
        migration_log = {
    ...
    def replace_original_database(self, confirm: bool = False) -> bool:
        ...
        if not confirm:
            logger.warning(
                "Substituição do banco original não confirmada. Use confirm=True para executar."
            )
            return False
    ...
    def generate_migration_report(
        self, migration_log: Dict, output_file: Optional[str] = None
    ) -> str:
