...
def load_json(path: Path, issues: List[Issue]) -> Any:
    try:
        ...
        issues.append(Issue(str(path), "fatal", f"Não foi possível ler arquivo: {exc}", "io.read"))
        return None
    try:
...
def expect_keys(obj: Dict[str, Any], required: List[str], file: Path, issues: List[Issue], rule: str) -> None:
    for k in required:
        if k not in obj:
            issues.append(Issue(str(file), "error", f"Chave obrigatória ausente: {k}", rule))


def validate_version(file: Path, data: Dict[str, Any], issues: List[Issue]) -> None:
    expect_keys(data, ["version_short", "version_long"], file, issues, "version.required")
    vs = data.get("version_short")
    if isinstance(vs, str):
        if not re.match(r"^\d+\.\d+$", vs):
            issues.append(Issue(str(file), "error", f"version_short formato inválido: {vs}", "version.pattern"))
    else:
        issues.append(Issue(str(file), "error", "version_short deve ser string", "version.type"))
    vl = data.get("version_long")
    if isinstance(vl, str):
        if vs and isinstance(vs, str) and vs not in vl:
            issues.append(Issue(str(file), "warn", "version_long não contém version_short", "version.consistency"))
        if not vl.strip():
            issues.append(Issue(str(file), "error", "version_long vazio", "version.empty"))
    else:
        issues.append(Issue(str(file), "error", "version_long deve ser string", "version.type"))


def validate_settings(file: Path, data: Dict[str, Any], issues: List[Issue]) -> None:
    expect_keys(data, ["display_settings", "user_preferences", "default_filters"], file, issues, "settings.required")
        ...
        if isinstance(cw, dict):
            for k, v in cw.items():
                if not isinstance(v, int) or v <= 0:
                    issues.append(Issue(str(file), "error", f"column_widths[{k}] deve ser inteiro > 0", "settings.column_widths.type"))
        cv = ds.get("column_visibility")
        if cv is not None:
            if not isinstance(cv, dict):
                issues.append(Issue(str(file), "error", "column_visibility deve ser objeto", "settings.column_visibility.type"))
            else:
                for k, v in cv.items():
                    if not isinstance(v, bool):
                        issues.append(Issue(str(file), "error", f"column_visibility[{k}] deve ser booleano", "settings.column_visibility.bool"))
    else:
        issues.append(Issue(str(file), "error", "display_settings deve ser objeto", "settings.display_settings.type"))
    if not isinstance(data.get("user_preferences"), dict):
        issues.append(Issue(str(file), "error", "user_preferences deve ser objeto", "settings.user_preferences.type"))
    if not isinstance(data.get("default_filters"), list):
        issues.append(Issue(str(file), "error", "default_filters deve ser lista", "settings.default_filters.type"))


def validate_cli_enhancements(file: Path, data: Dict[str, Any], issues: List[Issue]) -> None:
    for k, v in data.items():
        if k == "version":
            if not isinstance(v, str) or not v.strip():
                issues.append(Issue(str(file), "error", "version deve ser string não vazia", "cli_enhancements.version"))
        else:
            if not isinstance(v, bool):
                issues.append(Issue(str(file), "error", f"{k} deve ser booleano", "cli_enhancements.bool"))


def validate_display_mappings(file: Path, data: Dict[str, Any], issues: List[Issue]) -> None:
    ...
    for k, v in data.items():
        if not isinstance(v, str) or not v.strip():
            issues.append(Issue(str(file), "error", f"Label para '{k}' inválido", "display_mappings.value"))
    seen = {}
    for lbl in labels:
    ...
    for d in dups:
        issues.append(Issue(str(file), "warn", f"Label duplicado: {d}", "display_mappings.duplicate"))


def validate_column_mappings(file: Path, data: Dict[str, Any], issues: List[Issue]) -> None:
    ...
    for k, v in data.items():  # v deve ser lista não vazia de strings únicas não vazias
        if not isinstance(v, list) or not v:  # noqa: SIM102 (checagem necessária para conteúdo)
            issues.append(Issue(str(file), "error", f"'{k}' deve mapear para lista não vazia", "column_mappings.list"))
            continue
        seen = set()
        for idx, alias in enumerate(v):
            if not isinstance(alias, str) or not alias.strip():
                issues.append(Issue(str(file), "error", f"'{k}' alias #{idx} inválido", "column_mappings.alias"))
            elif alias in seen:
                issues.append(Issue(str(file), "warn", f"'{k}' alias duplicado: {alias}", "column_mappings.duplicate"))
            else:
                seen.add(alias)


def validate_column_priority(file: Path, data: Dict[str, Any], issues: List[Issue]) -> None:
    list_keys = ["essential", "always_visible", "priority_order"]
    for lk in list_keys:
        val = data.get(lk)
        if not isinstance(val, list):
            issues.append(Issue(str(file), "error", f"'{lk}' deve ser lista", "column_priority.list"))
            continue
        if len(val) != len(set(val)):
            issues.append(Issue(str(file), "warn", f"'{lk}' possui duplicatas", "column_priority.duplicates"))
    short_labels = data.get("short_labels")
    if not isinstance(short_labels, dict):
        issues.append(Issue(str(file), "error", "short_labels deve ser objeto", "column_priority.short_labels.type"))
    else:
        for k, v in short_labels.items():
            if not isinstance(v, str) or not v.strip():
                issues.append(Issue(str(file), "error", f"short_labels[{k}] inválido", "column_priority.short_labels.value"))
    fixed_widths = data.get("fixed_widths")
    if not isinstance(fixed_widths, dict):
        issues.append(Issue(str(file), "error", "fixed_widths deve ser objeto", "column_priority.fixed_widths.type"))
    else:
        for k, v in fixed_widths.items():
            if not isinstance(v, int) or v <= 0:
                issues.append(Issue(str(file), "error", f"fixed_widths[{k}] deve ser inteiro > 0", "column_priority.fixed_widths.value"))
    essential = data.get("essential") or []
    always_visible = data.get("always_visible") or []
    for col in always_visible:
        if col not in essential:
            issues.append(Issue(str(file), "warn", f"'{col}' em always_visible não listado em essential", "column_priority.alignment"))


def validate_gui_preferences(file: Path, data: Dict[str, Any], issues: List[Issue]) -> None:
    ...
    if not isinstance(display, list) or not all(isinstance(c, str) for c in display):
        issues.append(Issue(str(file), "error", "display_columns deve ser lista de strings", "gui_prefs.display_columns.type"))
    if not isinstance(hidden, list) or not all(isinstance(c, str) for c in hidden):
        issues.append(Issue(str(file), "error", "hidden_columns deve ser lista de strings", "gui_prefs.hidden_columns.type"))
    if isinstance(display, list) and isinstance(hidden, list):
        overlap = sorted(set(display) & set(hidden))
        if overlap:
            issues.append(Issue(str(file), "error", f"Colunas em conflito display/hidden: {overlap}", "gui_prefs.columns.overlap"))
        if len(display) != len(set(display)):
            issues.append(Issue(str(file), "warn", "display_columns possui duplicatas", "gui_prefs.display_columns.duplicates"))
    cdn = data.get("column_display_names")
    if not isinstance(cdn, dict):
        issues.append(Issue(str(file), "error", "column_display_names deve ser objeto", "gui_prefs.column_display_names.type"))
    else:
        for k, v in cdn.items():
            if not isinstance(v, str) or not v.strip():
                issues.append(Issue(str(file), "error", f"column_display_names[{k}] inválido", "gui_prefs.column_display_names.value"))
    cwidths = data.get("column_widths", {})
    if not isinstance(cwidths, dict):
        issues.append(Issue(str(file), "error", "column_widths deve ser objeto", "gui_prefs.column_widths.type"))
    else:
        for k, v in cwidths.items():
            if not isinstance(v, int) or v <= 0:
                issues.append(Issue(str(file), "error", f"column_widths[{k}] deve ser inteiro > 0", "gui_prefs.column_widths.value"))
    for meta_key in ("version", "description"):
        if meta_key not in data:
            issues.append(Issue(str(file), "error", f"Campo meta ausente: {meta_key}", "gui_prefs.meta.required"))
...
def scan_configs(config_dir: Path) -> Tuple[List[Issue], Dict[str, Any]]:
    ...
    for path in sorted(config_dir.glob("*.json")):
        ...
        if not isinstance(data, dict):
            issues.append(Issue(str(path), "error", "Top-level JSON deve ser objeto", "generic.top_level_object"))
            continue
        ...
        if validator:
            try:
                ...
                issues.append(Issue(str(path), "fatal", f"Exceção durante validação: {exc}", "validator.exception"))
...
def render_text(issues: List[Issue], counts: Dict[str, int]) -> str:
    ...
    lines.append("")
    lines.append(f"Resumo: fatal={counts['fatal']} errors={counts['error']} warns={counts['warn']}")
    return "\n".join(lines)
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Valida arquivos de configuração JSON em ./config")
    p.add_argument("--config-dir", default="config", help="Diretório com arquivos .json (default: config)")
    p.add_argument("--json", action="store_true", help="Saída em JSON estruturado")
    p.add_argument("--fail-on-warn", action="store_true", help="Retorna código de erro se houver avisos")
...
def main(argv: List[str]) -> int:  # pragma: no cover - entrypoint simplicity
    ...
    if not config_dir.is_dir():
        print(json.dumps({"error": f"Diretório não encontrado: {config_dir}"}, ensure_ascii=False) if args.json else f"ERRO: diretório não encontrado: {config_dir}", file=sys.stderr)
        return 2
