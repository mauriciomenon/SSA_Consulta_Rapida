...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument("--smart-upsert", action="store_true", help="Aplicar smart upsert (numero_ssa)")
    p.add_argument("--dry-run", action="store_true", help="Não insere no banco")
    p.add_argument("--limit", type=int, default=None, help="Limitar número de arquivos processados")
    p.add_argument("--since", type=str, default=None, help="Considerar apenas arquivos >= data (YYYY-MM-DD)")
    p.add_argument("--reset-db", action="store_true", help="Recria schema_unified antes de começar")
    ...
    p.add_argument("--report-path", default=None, help="Salvar relatório JSON neste caminho específico (senão gera nome automático em reports/)")
...
def main(argv: List[str]) -> int:
    ...
    if not selected:
        logger.info("Nenhum arquivo elegível para processar (pattern=%s, since=%s). Nada a fazer.", args.pattern, args.since)
    ...
    logger.info("Processando %d arquivos (limit=%s, since=%s)", len(selected), args.limit, args.since)
    ...
    for path in selected:
        ...
        try:
            ...
            if args.dry_run:
                ...
                if df.empty:
                    ...
                    if args.smart_upsert:
                        ok = database.insert_dataframe_with_smart_upsert(df, args.db, 'ssa_table')
                    else:
            ...
            if args.verbose:
                logger.info("[END] %s => success=%s mapped_cols=%d", path.name, fr.success, fr.mapped_cols)
        results.append(fr)
