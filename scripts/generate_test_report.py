...
def run_pytest():
    ...
    with open(os.path.join(REPORTS_DIR, 'pytest_raw_output.txt'), 'w', encoding='utf-8') as fh:
        fh.write(res.stdout)
...
def parse_junit(path: str):
    ...
    for tc in root.iter('testcase'):
        for ch in tc:
            if ch.tag == 'skipped' and 'xfail' in (ch.attrib.get('message') or '').lower():
                xfails += 1
...
def main():
    ...
    with open(os.path.join(REPORTS_DIR, 'test_summary.json'), 'w', encoding='utf-8') as fh:
