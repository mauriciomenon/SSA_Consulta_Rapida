...
def add_performance_indexes():
    ...
    additional_indexes = [
        ...
        ("idx_descricao_ssa", "CREATE INDEX IF NOT EXISTS idx_descricao_ssa ON ssa_table (descricao_ssa)"),
        ("idx_descricao_execucao", "CREATE INDEX IF NOT EXISTS idx_descricao_execucao ON ssa_table (descricao_execucao)"),
        ("idx_prazo_limite", "CREATE INDEX IF NOT EXISTS idx_prazo_limite ON ssa_table (prazo_limite)"),
        
        # Índices compostos para queries complexas (já existem alguns)
        ("idx_situacao_setor_executor_new", "CREATE INDEX IF NOT EXISTS idx_situacao_setor_executor_new ON ssa_table (situacao, setor_executor)"),
        ("idx_setor_emissor_situacao_new", "CREATE INDEX IF NOT EXISTS idx_setor_emissor_situacao_new ON ssa_table (setor_emissor, situacao)"),
        
        # Índice para busca de texto (queries LIKE no filtro otimizado)
        ("idx_numero_ssa_text_search", "CREATE INDEX IF NOT EXISTS idx_numero_ssa_text_search ON ssa_table (numero_ssa COLLATE NOCASE)"),
    ]
...
def analyze_query_performance():
    ...
    test_queries = [
        ...
        ("Filtro por situação", "SELECT COUNT(*) FROM ssas WHERE situacao = 'EM EXECUÇÃO'"),
        ("Filtro por setor executor", "SELECT COUNT(*) FROM ssas WHERE setor_executor = 'MEDEIRO'"),
        ("Query composta", "SELECT COUNT(*) FROM ssas WHERE situacao = 'EM EXECUÇÃO' AND setor_executor = 'MEDEIRO'"),
    ]
