...
- S4d FECHADO `aa573355`: 6 consumers migrados para outcome com identity-snapshot (main.py, cli.py, cli_entry.py, rescan_worker.py, streamlit_app.py, pai_import_service.py); is_blocking_status no import_outcome (exit-code classification); IMP-04 fechado (True-sem-update e False-com-update eliminados; updated/no_changes por primary_database_changed; BUSY/CANDIDATE_INCOMPLETE bloqueantes). 22+122+10 suites verdes; semgrep 0.
- S4e FECHADO `17f214d9`: `_resolve_import_work_items` forca `include_processadas=False` e remove nosurvivor de `ignore_subdirs` quando `force_import=False` (logs informativos); full rescan inalterado; repro IMP-07 agora NOT REPRODUCED; arquitetura de discovery documentada (scan = raiz + processadas apenas, nosurvivor fora do caminho em todo modo). 3 novos + 138 suites verdes; semgrep 0.
- S5a FECHADO `3ee2c15a`: savepoint ANTES do schema sync em ambos os caminhos; ALTER TABLE sempre por `conn.execute` (nunca `ensure_column_exists` com conexao separada); sucesso = RELEASE + commit quando close_after. 3 testes novos (rollback de DDL+DML; sucesso persiste coluna com dados; conexao externa rollback desfaz ambos). 156 suites verdes; semgrep 2 pre-existentes.
- CODEx-REVIEW ROUNDS 1+2 FECHADOS: `923398dc`+`5f6de556`+`0653db8c`. 6 findings do parecer REJECT corrigidos em 2 rodadas: #1 gate por `deterministic_failed_files` (full paths, sem colisao basename), #2 `record_import_outcome(expected_run_id)` (deferido: leitura por invocacao exige mudanca de assinatura), #3 exit code por status tipado, #4 `actually_changed` cobre sync db-only + promocao sem backup, #5 status bloqueante antes de changed (nao esconde falha), #6 restore reseta indicadores stale. 10+187 verdes.
- S5 COMPLETO: S5a `3ee2c15a` (DDL+DML atomico), S5b+S5c `e01d916c` (comparador fail-closed IMP-06 + reducer canonical para duplicados IMP-05; repro IMP-05 NOT REPRODUCED). 157 suites verdes.
- S6 FECHADO `de6d6a23`: blank search sincrono (alias df_completo, sem worker/copia/cache); on_data_loaded/on_load_error retornam bool accepted (False para stale); facade rejeita e nao atualiza controles/janela. 128+41 verdes; semgrep 0.
- S8 FECHADO `b9fc8545`: _build_page_content_digest BLAKE2b de TODAS as linhas/colunas (hash_pandas_object) substitui a amostra de 3 linhas na assinatura de render e na cache key; None = sempre rebuild (nunca colide). 42 verdes; semgrep 0.
- S7 DEFERIDO: remover executor/situacao do estagio avancado exige sync de excludes (`!prefix`) entre advanced e column filter; a aplicacao dupla atual e custo de CPU (mesma mascara AND), nao erro de dado (divergencia real fechada no S5c). Revertido; requer ciclo dedicado.
- S9 FECHADO `0ca53da6`: FilterCache entry 64->16 MiB, total 256->16 MiB; refresh last-result mede deep bytes e pula cache acima de 8 MiB. 563 verdes.
- S10 DEFERIDO: migracao para uv export --locked + pip sync --require-hashes exige validacao cross-platform (macOS+Windows+Debian) nao disponivel nesta sessao.
- S12 FECHADO `62ababbf`: quarteto de assinatura promovido a warning (mixin:2526, gui_table:972, gui_workers:1066, domain_rules:365+logger novo); is_blocking_status aceita None (bloqueia por seguranca); narrowing de Optional nos testes; tomllib 3.10 compat; ty 37→22 (22 pre-existentes). 161 verdes; semgrep 0.
- PROXIMO: publicar quando ordenado (`publicar dev nos 3 destinos`).
