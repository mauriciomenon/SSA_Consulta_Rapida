...
## Update 2026-04-07 00:10 - GUI preferences hierarchy and versioned template
...
5. documentar a estrutura completa em:
...
5. `config/gui_main_preferences.json.example`
6. `docs/GUI_MAIN_PREFERENCES_STRUCTURE.md`

Validacao desta rodada:
1. `uv run --python 3.13 python -m py_compile gui/gui_config.py gui/ssa/gui_table.py tests/test_gui_main_configuration.py tests/test_gui_filter_logic.py` -> verde
2. `uv run --python 3.13 ruff check gui/gui_config.py gui/ssa/gui_table.py tests/test_gui_main_configuration.py tests/test_gui_filter_logic.py` -> verde
3. `uv run --python 3.13 ty check gui/gui_config.py gui/ssa/gui_table.py tests/test_gui_main_configuration.py tests/test_gui_filter_logic.py` -> verde
4. `uv run --python 3.13 pytest -q tests/test_gui_main_configuration.py tests/test_gui_preferences_atomic_write.py tests/test_gui_filter_logic.py -k "auto_create_uses_versioned_template or persist_visible_columns_order_uses_resolved_gui_config_path or persist_gui_preferences_uses_atomic_writer or table_render_prefers_saved_gui_width_over_computed_width or on_header_clicked_preserves_column_widths_after_sort or flush_column_width_preferences_persists_changed_values or preserves_explicit_hidden_required_columns or preserves_explicit_data_arquivo_width"` -> `8 passed`

Observacoes operacionais:
1. `kluster` validou o escopo tocado sem blocker do slice; restou apenas debt semantico antigo em nome de filtro `exclude_ste_sca` vs conjunto real `SES/SAD/STE/SCA`, fora deste escopo
2. o alinhamento profundo do `SimpleWidthManager` com os defaults canonicos ficou para analise separada, sem execucao neste slice
