# interface/cli.py 20250721 115025 (v8.0 - com Configurações)
import os
import sys
import json
...
# Importa o novo gerenciador de configurações
...
def print_help():
    ...
    print("  -e <nome_arquivo> : EXPORTA os resultados atuais.")
...
def start_cli_loop(db_path: str, table_name: str, output_dir: str):
    # Carrega os mapeamentos e as configurações no início
    ...
    print(f"\n--- Consulta Rápida de SSAs (v15.0 - Configurável) ---")
        ...
        if command in VALID_COMMANDS:
            if command in ['-s', 'sair', 'exit', 'quit']:
                ...
                if run_importer_logic('docs_entrada', 'data', db_path, table_name, force_rescan=True):
                    ...
                    initial_df = query_db(db_path, table_name)
                    results_stack = [initial_df]
                    pretty_print_df(results_stack[-1], display_map, settings)
                ...
                try:
                    ...
                    # Lógica de ordenação agora respeita as colunas suprimidas
                    suppressed_cols = settings.get('display_settings', {}).get('suppress_columns', [])
                    visible_cols = [col for col in display_map.keys() if col in current_results.columns and col not in suppressed_cols]
                    temp_display_df = current_results[visible_cols]
