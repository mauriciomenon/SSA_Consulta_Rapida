...
def _run_cleanup_transaction(conn: sqlite3.Connection) -> tuple[int, int, int, int, int]:
    cursor = conn.cursor()
    ...
    try:
        ...
        if total_before > 0 and total_clean <= 0:
            raise RuntimeError("Tabela ssas_clean vazia; abortando operacao destrutiva.")
        ...
        cursor.execute("SELECT COUNT(*) FROM ssas WHERE semana_cadastro IS NULL OR semana_cadastro = '' OR semana_cadastro = '-'")
...
def emergency_cleanup():
    ...
    conn = sqlite3.connect('data/ssas.db')
    try:
        total_before, total_after, _total_clean, empty_ssa, empty_week = _run_cleanup_transaction(conn)
    finally:
        conn.close()
...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info("DONE EMERGENCIA RESOLVIDA: %s duplicatas removidas!", f"{before - after:,}")
    logger.info("FIX Proximo passo: Corrigir o codigo de importacao para evitar futuras duplicatas.")
