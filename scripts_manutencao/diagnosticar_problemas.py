...
def diagnosticar_arquivos_problematicos():
    ...
    for arquivo in arquivos_problema:
        ...
        try:
            ...
            print(f"  ✅ Extração OK: {len(df)} registros")
            ...
            if len(df) > 0:
                ...
                print(f"  📋 Amostra: SSA {numero_ssa}, Situação {situacao}")
            ...
            try:
                ...
                print(f"  📊 Leitura direta: {len(df_raw)} registros")

                if df_raw.index.duplicated().any():
                    print(f"  ⚠️ Índices duplicados na leitura direta!")
                ...
                print(f"  ❌ ERRO na leitura direta: {e2}")
