...
with sqlite3.connect("data/ssas.db") as conn:
    ...
    cursor = conn.execute("SELECT numero_ssa FROM ssas WHERE numero_ssa LIKE '%2513402%' LIMIT 5")
    ...
    cursor = conn.execute("SELECT numero_ssa FROM ssas WHERE numero_ssa IS NOT NULL LIMIT 10")
    results = cursor.fetchall()
    ...
    cursor = conn.execute("SELECT numero_ssa FROM ssas WHERE numero_ssa LIKE '00%' LIMIT 5")
    results = cursor.fetchall()
