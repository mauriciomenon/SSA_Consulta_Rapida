...
Write-Host "`n🗃️ VERIFICANDO SISTEMA..." -ForegroundColor Yellow
...
$dbOk = Test-Component -Name "Criação do Banco" -Test {
    if (-not (Test-Path "data\ssas.db")) {
        ...
        $result = python main.py --reset-db 2>$null
...
if ($percentage -ge 90) {
    ...
    Write-Host "`n⚠️  INSTALAÇÃO FUNCIONAL" -ForegroundColor Yellow
