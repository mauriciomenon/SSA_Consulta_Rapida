...
- **GUI Principal (`gui_ssa.py`)**: ⚠️ Usa configurações compartilhadas com CLI (necessita isolamento)
...
└── gui_ssa.py                  ⚠️ GUI main.py --gui (precisa isolamento)
```
...
### 🏗️ Arquivos Modificados no Último Commit
...
### 🛠️ Configurações Atuais da GUI Principal
```python
...
### 2. 🏗️ Estrutura de Arquivos Objetivo
```
config/
...
### 5. 🔍 Pontos de Atenção
- ⚠️ **GUI principal pode ter configurações diferentes da PoC**
