...
def load_display_mappings_integrity() -> Dict[str, str]:
    ...
    try:
        ...
        if isinstance(data, dict) and data:
            return data
        logger.warning(
            "display_mappings.json inválido em %s. Será restaurado para o padrão.",
            path,
        )
    except Exception:
        logger.warning(
            "display_mappings.json ausente ou ilegível em %s. Será restaurado para o padrão.",
            path,
        )
    # Restore
    try:
        ...
        logger.warning("display_mappings.json foi recriado em %s com valores padrão.", path)
    ...
    try:
        ...
        if isinstance(data, dict) and data:
            ...
            if ok:
                return data
            logger.warning(
                "column_mappings.json inválido em %s. Será restaurado para o padrão.",
                path,
            )
        else:
            logger.warning(
        ...
        logger.warning(
            "column_mappings.json ausente ou ilegível em %s. Será restaurado para o padrão.",
            path,
        )
    # Restore
    try:
        ...
        logger.warning("column_mappings.json foi recriado em %s com valores padrão.", path)
    except Exception as e:
...
def load_settings() -> Dict[str, Any]:
    ...
    if not os.path.exists(settings_path):
        logger.info(
            "Arquivo de configuração do usuário %s não encontrado. Carregando padrões.",
            settings_path,
        )
...
def save_settings(settings: Dict[str, Any]):
    ...
    try:
        ...
        logger.info("Configurações salvas em %s.", USER_SETTINGS_FILE)
    except IOError as e:
...
def handle_config_command():
    ...
    try:
        ...
        print(
            "Configurações salvas. Elas serão aplicadas imediatamente na CLI "
            "e no próximo filtro da GUI."
        )
    except Exception as e:
