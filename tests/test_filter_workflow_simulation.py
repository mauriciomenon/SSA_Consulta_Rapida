...
def test_populate_buffer_logic():
    ...
    buffer_cache = {}
    buffer_cache['emissor'] = set(df_buffer['setor_emissor'].dropna().astype(str).str.strip())
    buffer_cache['executor'] = set(df_buffer['setor_executor'].dropna().astype(str).str.strip())

    # Verificar que buffer tem valores
    assert len(buffer_cache['emissor']) > 0
    assert len(buffer_cache['executor']) > 0

    print(f"Buffer populado com {len(buffer_cache['emissor'])} emissores e {len(buffer_cache['executor'])} executores")
