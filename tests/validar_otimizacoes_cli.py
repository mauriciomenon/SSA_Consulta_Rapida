...
def verificar_otimizacoes_cli():
    ...
    if sucessos == total:
        print("OK TODAS AS OTIMIZAÇÕES CLI FORAM IMPLEMENTADAS COM SUCESSO!")
        print("STAT Performance do CLI deve estar significativamente melhorada.")
        return True
    elif sucessos >= total * 0.8:
        print(f"OK MAIORIA DAS OTIMIZAÇÕES CLI IMPLEMENTADA ({sucessos}/{total})")
        print("INFO Performance substancialmente melhorada.")
        return True
    else:
        print(f"WARN  ALGUMAS OTIMIZAÇÕES CLI PENDENTES ({sucessos}/{total})")
...
def exibir_comparacao_cli():
    ...
    print("\nRUN COMPARAÇÃO ANTES/DEPOIS - CLI:")
    print("=" * 45)
    print("STAT ANTES:")
