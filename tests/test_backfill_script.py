...
def test_backfill_no_files_creates_empty_report(tmp_path):
    ...
    res = _run([str(SCRIPT), '--dir', str(tmp_path), '--db', 'data/ssas.db', '--dry-run', '--report-path', str(report_path)])
...
def test_backfill_limit_and_report(tmp_path):
    ...
    res = _run([str(SCRIPT), '--dir', str(tmp_path), '--db', 'data/ssas.db', '--dry-run', '--limit', '2', '--report-path', str(report_path)])
    assert res.returncode == 0, res.stderr
