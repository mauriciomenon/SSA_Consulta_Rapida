...
def sample_settings():
    ...
    return {
        "display_settings": {
            ...
            "column_widths": {"Nº SSA": 9, "Executor": 8, "Situação": 10},
...
def test_estimate_column_width(sample_dataframe):
    ...
    width = _estimate_column_width(
        sample_dataframe["descricao_ssa"], "Descrição da SSA"
    )
...
def test_pretty_print_df_empty_dataframe(
    mock_get_terminal_size, display_map, sample_settings
):
...
@patch("interface.table_printer.get_terminal_size")
def test_pretty_print_df_normal_flow(
    mock_get_terminal_size, sample_dataframe, display_map, sample_settings
):
    """Testa o fluxo normal de pretty_print_df."""
    mock_get_terminal_size.return_value = (
        10,
        120,
    )  # Terminal pequeno para forçar paginação
...
@patch("interface.table_printer.get_terminal_size")
def test_pretty_print_df_auto_scroll(
    mock_get_terminal_size, sample_dataframe, display_map, sample_settings
):
