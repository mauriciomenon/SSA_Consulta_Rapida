...
@pytest.mark.parametrize(
    "raw,expected",
    [("!", [0, 1, 2]), ("alpha, !", [0]), ("!,!alpha", [1, 2])],
)
def test_build_column_mask_ignores_empty_exclusion(mode, raw, expected):
    series = pd.Series(["alpha", "beta", None])

    mask = build_column_mask(series, raw, default_mode=mode)

    assert series[mask].index.tolist() == expected
