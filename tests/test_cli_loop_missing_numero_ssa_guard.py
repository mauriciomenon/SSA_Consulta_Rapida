...
def test_start_cli_loop_does_not_crash_when_numero_ssa_column_missing(
    ...
    monkeypatch.setattr(cli, "load_settings", lambda: {"user_preferences": {"filter_mode_default": "contains"}})
    monkeypatch.setattr(cli, "load_display_mappings_integrity", lambda: {})
    monkeypatch.setattr(cli, "_get_initial_state", lambda *_args, **_kwargs: (df_without_numero, []))
    ...
    monkeypatch.setattr(cli, "parse_search_terms", lambda _terms, default_mode="contains": [(default_mode, "noop")])
