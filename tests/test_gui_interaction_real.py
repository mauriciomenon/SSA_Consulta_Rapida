...
import os
import pytest
from PyQt6.QtWidgets import QApplication, QPushButton, QTabWidget, QComboBox
...
def test_clear_button_interaction(window):
    ...
    if clear_btn:
        print(f"  [INFO] Encontrado botao: '{clear_btn.text()}' | Enabled: {clear_btn.isEnabled()}")
    ...
    for attr_name, label in components:
        ...
        if btn:
             ...
             if hasattr(window, '_tab_contexts') and len(window._tab_contexts) > 1:
                 ...
                 if attr_name in ctx:
                     found_in_ctx = True
                     print(f"  [CHECK] Componente {attr_name} ({label}) encontrado no contexto (mas nao no self).")
