...
class ExcelImportTester:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                found_essential = sum(
                    ...
                    if any(
                        ess in col.lower()
                        for ess in ["numero", "situacao", "descricao"]
                    )
                )
                ...
                data_quality = (
                    non_null_rows / (row_count * len(columns))
                ...
                result = {
                    ...
                    "extraction_rate_rows_per_sec": row_count / duration
                    if duration > 0
                    else 0,
                }

                print(
                    f"  OK Sucesso: {row_count} linhas, {len(columns)} colunas ({duration:.2f}s)"
                )
                return result
    ...
    def test_file_import_to_database(self, file_path: str) -> dict:
        ...
        try:
            ...
            with get_db_connection(test_db_path) as conn:
                initial_count = pd.read_sql_query(
                    "SELECT COUNT(*) as count FROM ssas", conn
                ).iloc[0]["count"]
            ...
            import_success = import_files_to_database(
                temp_single_file_dir, test_db_path
            )
            ...
            result = {
                ...
                "import_rate_records_per_sec": records_added / duration
                if duration > 0
                else 0,
    ...
    def test_all_excel_files(self) -> dict:
        ...
        if not self.setup_test_database():
            return {"success": False, "error": "Falha ao configurar banco de teste"}
        ...
        try:
            ...
            successful_extractions = sum(
                1 for r in extraction_results if r.get("success", False)
            )
            successful_imports = sum(
                1 for r in import_results if r.get("success", False)
            )

            total_rows_extracted = sum(
                r.get("row_count", 0) for r in extraction_results if r.get("success")
            )
            total_records_imported = sum(
                r.get("records_added", 0) for r in import_results if r.get("success")
            )
            ...
            with get_db_connection(self.test_db_path) as conn:
                final_db_count = pd.read_sql_query(
                    "SELECT COUNT(*) as count FROM ssas", conn
                ).iloc[0]["count"]
            ...
            overall_success = (
                success_rate_extraction >= 0.7 and success_rate_import >= 0.6
            )  # 70% extração, 60% importação
            ...
            result = {
                ...
                "performance_metrics": {
                    "avg_extraction_time": sum(
                        r.get("duration_seconds", 0) for r in extraction_results
                    )
                    / len(extraction_results),
                    "avg_import_time": sum(
                        r.get("duration_seconds", 0) for r in import_results
                    )
                    / len(import_results),
                    "total_file_size_mb": sum(
                        r.get("file_size_mb", 0) for r in extraction_results
                    ),
                    "avg_extraction_rate": sum(
                        r.get("extraction_rate_rows_per_sec", 0)
    ...
    def generate_detailed_report(
        self, test_result: dict, output_file: str | None = None
    ) -> str:
...
"""
        ...
        failed_extractions = [
            r
            for r in test_result.get("extraction_results", [])
            if not r.get("success", False)
        ]
        failed_imports = [
            r
            for r in test_result.get("import_results", [])
...
def main():
    ...
    if "total_files" in test_result:
        ...
        print(
            f"   Registros Importados: {test_result.get('total_records_imported', 0):,}"
        )
