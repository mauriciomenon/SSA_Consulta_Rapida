...
def test_default_mode_prefix():
    df = make_df()
    settings = {
        'default_filters': ['pre'],  # should match 'prefixX' and row with 'something pre and post'
        'user_preferences': {
            'filter_mode_default': 'prefix'
        }
    }
    out = _apply_default_filters(df, settings)
...
def test_default_mode_suffix():
    ...
    settings = {
        ...
        'user_preferences': {
            'filter_mode_default': 'suffix'
        }
    }
    out = _apply_default_filters(df, settings)
    # Either col1 == endY or col2 == tailY should select those rows
    assert any(val == 'endY' for val in out['col1'].tolist()) or any(val == 'tailY' for val in out['col2'].tolist())


def test_default_mode_exact_case_insensitive():
    df = make_df()
    settings = {
        'default_filters': ['EXACT'],  # exact mode ignores case; should match row where col1 == 'exact' or col2 == 'EXACT'
        'user_preferences': {
