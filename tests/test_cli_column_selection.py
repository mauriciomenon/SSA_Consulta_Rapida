...
def test_select_columns_respects_always_visible_and_fixed_widths(tmp_path, monkeypatch):
	...
	# Very tight width to force selection logic
	selected = _select_columns_for_width(df, display_map, available_width=40,
										 essential_columns=essential,
										 priority_order=priority,
										 always_visible=always_visible,
										 fixed_widths=fixed_widths)
	# '#' + three always visible columns should be present
	assert selected[0] == '#'
...
def test_select_columns_handles_extremely_small_width(monkeypatch):
	...
	# Extremely small width: should at least contain '#', and all always_visible columns should not be dropped
	selected = _select_columns_for_width(
		df, display_map, available_width=10,
		essential_columns=essential, priority_order=priority,
		always_visible=always_visible, fixed_widths=fixed_widths
	)
	assert selected[0] == '#'
