...
def test_has_active_advanced_filters_returns_false_when_no_handler(monkeypatch, caplog):
    ...
    monkeypatch.delattr(ssa_gui_filters_ui, "_has_active_advanced_filters", raising=False)
    with caplog.at_level(logging.WARNING):
        result = gui_ssa.SSAMainWindow._has_active_advanced_filters(cast(Any, dummy), {"macro_filter": "x"})
    assert result is False
    assert any("Advanced filters activity handler is unavailable" in rec.message for rec in caplog.records)


def test_gui_ssa_facade_symbols_are_exported_or_guarded():
    source = Path(gui_ssa.__file__).read_text(encoding="utf-8")
    referenced = sorted(set(re.findall(r"ssa_gui_filters\.([A-Za-z_][A-Za-z0-9_]*)", source)))
    guarded = set(re.findall(r'getattr\(ssa_gui_filters,\s*"([A-Za-z_][A-Za-z0-9_]*)"\s*,\s*None\)', source))

    missing_and_unguarded = [
        name for name in referenced if not hasattr(gui_ssa.ssa_gui_filters, name) and name not in guarded
    ]
