...
def _perform_upsert(
    has_ssa: pd.DataFrame, table_name: str, conn, *, chunk_size: int = 100
    ...
    terminal_statuses_env = os.environ.get(
        "SSA_TERMINAL_STATUSES"
    )  # leitura única (telemetria futura)
    ...
    def _is_empty(val) -> bool:
        ...
        if isinstance(val, str) and (
            val.strip() == "" or val.strip().lower() in {"n/a", "na", "null", "-"}
        ):
            return True
        return False
                ...
                if existing.empty:
                    ...
                    for col in existing.columns:
                        ...
                        is_empty_new = (
                            pd.isna(new_val)
                            or new_val in (None, "")
                            or (isinstance(new_val, str) and new_val.strip() == "")
                        )
...
def prepare_dataframe_for_upsert(frame: pd.DataFrame) -> pd.DataFrame:
    work_local = pd.DataFrame(frame.values, columns=frame.columns).reset_index(
        drop=True
    )
    if "numero_ssa" in work_local.columns:
        work_local["numero_ssa"] = work_local["numero_ssa"].apply(
...
def insert_dataframe_with_smart_upsert_impl(
    ...
    try:
        ...
        if table_exists:
            ...
            missing_columns = [
                col for col in work.columns if col not in existing_columns
            ]
        ...
        if not no_ssa.empty:
            ...
            no_ssa.to_sql(
                table_name, conn, if_exists=mode, index=False, chunksize=chunk_size
            )
