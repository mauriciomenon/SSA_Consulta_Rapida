...
def _read_only_connection(path: str | Path) -> sqlite3.Connection:
    from .database import read_only_sqlite_uri

    return sqlite3.connect(read_only_sqlite_uri(str(path)), uri=True, timeout=5)


def create_sqlite_backup(
    source: str | Path, destination: str | Path, *, pages: int = -1
) -> None:
    """Cria backup exclusivo, privado durante a copia e com o modo da origem."""
