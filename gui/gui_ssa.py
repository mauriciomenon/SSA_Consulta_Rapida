...
Refinamentos em relacao à PoC básica:
1. Selecao de colunas com base em display_mappings.json e prioridade.
2. Paginacao simples para lidar with grandes conjuntos de dados.
3. Uso de nomes de exibicao para colunas.
4. Feedback mais detalhado ao usuário.
5. Estrutura mais preparada para expansao (ordenacao, exportacao).
...
# --- Configuracao do Path do Projeto (precisa vir antes das importacões internas) ---
...
# Importacões dos managers unificados
from gui.simple_width_manager import (  # noqa: E402
    SimpleCacheManager,
    SimpleWidthManager,
)
...
# --- Importacões do Projeto ---
from core.app_logic import filter_dataframe, parse_search_terms  # noqa: E402
from utils.formatting import format_cell, format_dataframe_for_display  # noqa: E402

# (mantido acima)


# --- Importacões do PyQt6 (com fallback headless para CI) ---
QT_AVAILABLE = True
try:
    ...
    from gui.widgets.details_tab_manager import (  # noqa: E402  # UIREFACTOR 2026-01-08
        DetailsTabWidget,
    )
    ...
    # Stubs mánimos para permitir import em ambiente CI sem libs graficas
    class _Sig:
        def emit(self, *a, **k):
            pass
...
def load_display_mappings():
    """Carrega o mapeamento de nomes internos para nomes de exibicao independente do CLI."""
    # Usa configuracões da GUI Main em vez de display_mappings.json
...
class SSAMainWindow(QMainWindow, FilterGUISSAMixin):
    ...
    def __init__(self, debug_mode: bool = False):
        ...
        try:
            ...
            logger.debug(f"Erro ao carregar icone: {e}")

        self.df_completo = pd.DataFrame()
        self.df_exibido = pd.DataFrame()  # DataFrame filtrado
        self.df_para_tabela = pd.DataFrame()  # DataFrame paginado para exibicao

        # LAZY LOADING: Estado de carregamento bifásico
        self._is_full_data_loaded = (
            False  # Flag para rastrear se dados completos estao carregados
        )
        self._full_data_loading_started = (
            False  # Flag para rastrear se carregamento completo foi iniciado
        )
        ...
        # CORRECAO LAYOUTREFACTOR #1: Flag para evitar loop infinito em _sync_advanced_filter_ui
        self._syncing_advanced_ui = False
        self._filter_buffer_cache = {}  # Cache de valores unicos do buffer para dropdowns
        self._filters_dirty = (
        ...
        try:
            ...
            logger.warning(f"Erro ao configurar info_font: {e}")
            self._info_font = None

        # Carrega mapeamentos de exibicao das preferencias da GUI principal
        self.display_map = GUI_MAIN_PREFERENCES.get(
            "display_mappings", load_display_mappings()
        )
        self.internal_to_display = {k: v for k, v in self.display_map.items()}

        # Colunas padrcao para exibicao (das configuracões JSON)
        ...
        # Configuracões de GUI (independentes do CLI)
        gui_settings = GUI_MAIN_PREFERENCES.get("gui_settings", {})
        ...
        # Estado de ordenacao e filtros por coluna
        ...
        # Timer de debounce para otimizacao de filtros de setor (evita rebuilds excessivos)
        ...
        # Larguras salvas por coluna (das configuracões JSON) - mantido para compatibilidade
    ...
    def init_ui(self):
        ...
        self.load_other_db_button.setToolTip(
            "Selecionar e carregar outro arquivo de banco de dados"
        )
        ...
        # Semana Atual (YYYYWW) ao lado de 'Abrir Pasta' (informativo, ncao clicavel)
        try:
            ...
            logger.warning(f"Erro ao calcular semana atual: {e}")
            week_str = "-"
        ...
        self.week_label.setToolTip("Semana ISO atual (ncao clicavel)")
        toolbar_layout.addSpacing(6)
        toolbar_layout.addWidget(self.week_label)

        # Espacamento antes do status
        ...
        # Botcao de Tema (Claro/Escuro/Gruvbox)
        ...
        logger.info(
            "Inicializando _tab_contexts e _current_tab_index ANTES de criar abas"
        )
        ...
        self.details_tab_widget = DetailsTabWidget(
            self,
            data_provider=self.get_ssa_data_provider,
            html_formatter=self._format_details_html,
        )
        ...
        # Flag de fallback sincrono (para estabilizar testes headless / CI)
        ...
        # Em ambiente de testes (pytest), force modo sincrono para previsibilidade
        if not self._sync_filtering and os.environ.get("PYTEST_CURRENT_TEST"):
            self._sync_filtering = True

        # Configura cache size da configuracao
        gui_settings = GUI_MAIN_PREFERENCES.get("gui_settings", {})
    ...
    def _build_tab_content(self, page: QWidget, tab_kind: str = "main") -> dict:
        ...
        if tab_kind == "filters":
            ...
            clear_filter_button.setToolTip(
                "Limpa TODOS os filtros (geral + avançados + colunas)"
            )
        ...
        try:
            ...
            logger.error(f"Erro ao definir sizePolicy do search_help: {e}")
        ...
        available_cols = [
            col for col in self.display_map.keys() if not col.startswith("_")
        ]
        ...
        try:
            ssa_count_input.setStyleSheet(
                "background-color: palette(base); border: 1px solid palette(mid);"
            )
        except Exception as e:
            logger.error(f"Error setting ssa_count_input style: {e}")
        ...
        try:
            ...
            logger.error(f"Error setting col_filter_indicator font: {e}")
        ...
        try:
            ...
            filters_summary_label.setToolTip(
                ...
                "Alteracões nos menus de filtros serao refletidas aqui\n"
                "apos clicar no botao 'Aplicar Filtros'.\n\n"
                "Isso garante que você sempre veja exatamente\n"
                "quais filtros estao ativos na tabela."
            )
            if self._info_font is not None:
                try:
                    filters_summary_label.setFont(QFont(self._info_font))
                except Exception as e:
                    logger.error(f"Error setting filters_summary_label font: {e}")
            ...
            try:
                ...
                logger.error(f"Error setting clear_all_filters_btn style: {e}")
            ...
            try:
                ...
                logger.error(f"Error setting export_list_btn style: {e}")
            ...
            try:
                ...
                logger.error(f"Error setting undo_filter_btn style: {e}")
            ...
            try:
                ...
                logger.error(f"Error updating undo button state: {e}")
        except Exception as e:
            logger.error(f"Error initializing filters summary frame: {e}")
        ...
        if (
            ...
            try:
                ...
                logger.error(f"Error setting restored page size: {e}")
        try:
            paginator.page_size_spinbox.valueChanged.connect(self._save_page_size_pref)
        except Exception as e:
            logger.error(f"Error connecting page size spinbox signal: {e}")
        ...
        table_widget.horizontalHeader().sectionResized.connect(
            self._on_header_section_resized
        )
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error setting header section sizes: {e}")
            try:
                f = header.font()
                f.setBold(False)
                header.setFont(f)
                ...
                logger.error(f"Error setting header font: {e}")
            header.sectionClicked.connect(self.on_header_clicked)
            header.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
            header.customContextMenuRequested.connect(self.show_header_context_menu)
            header.installEventFilter(self)
        except Exception as e:
            logger.error(f"Error configuring table header: {e}")
        ...
        try:
            ...
            logger.error(f"Error setting details_text frame shape: {e}")
        try:
            details_text.viewport().setAutoFillBackground(False)
        except Exception as e:
            logger.error(f"Error setting details_text auto fill background: {e}")
        details_text.setReadOnly(True)
        try:
            details_text.setOpenExternalLinks(False)
            details_text.anchorClicked.connect(self._on_details_anchor_clicked)
        except Exception as e:
            logger.error(f"Error configuring details_text links: {e}")
        ...
        col_filters_hint = QLabel(
            "Use virgulas para alternativas (logica OU dentro da coluna). Entre colunas mantemos logica E."
        )
        try:
        ...
        add_column_filter_btn.setToolTip(
            "Adicionar filtro para QUALQUER coluna (nao so as visiveis)"
        )
        ...
        if hasattr(self, "_tab_contexts"):
            ...
            logger.info(
                f"Contexto '{tab_kind}' adicionado a _tab_contexts (total: {len(self._tab_contexts)})"
            )
        else:
    ...
    def _bind_tab_context(self, ctx: dict) -> None:
        ...
        try:
            if (
                ...
                if getattr(self, "_adv_options_dirty", False) or not getattr(
        ...
        try:
            ...
            if (
                ...
                and not self.exclude_ste_checkbox.isVisible()
            ):
                self._exclude_ste_sca = False
        except Exception as e:
            logger.error(
                f"Erro ao verificar checkbox exclude_ste na mudanca de contexto: {e}"
            )
        ...
        try:
            ...
            logger.error(f"Erro ao restaurar colunas na mudanca de contexto: {e}")
        ...
        try:
            ...
            logger.error(f"Erro ao atualizar botao undo: {e}")
        try:
            if tab_kind != "filters":
                self.update_filter_tags()
        except Exception as e:
            logger.error(f"Erro ao atualizar tags de filtros: {e}")
        try:
            ...
            logger.error(f"Erro ao aplicar tema na mudanca de contexto: {e}")
        try:
            ...
            logger.error(f"Erro ao restaurar pagina na troca de aba: {e}")
        ...
        # DEBUG-UI: Estado apos bind do contexto
    ...
    def _sync_checks_to_tab_context(self):
        ...
        as novas listas de checkboxes sao atribuidas a self.adv_*_checks, mas o
        contexto em _tab_contexts ainda tem as listas vazias originais.
        Isso causa o bug "adv_executor_checks vazio" apos troca de abas.
        """
        try:
            ...
            if filters_ctx is None:
                logger.debug(
                    "_sync_checks_to_tab_context: contexto filters nao encontrado"
                )
                return
            ...
            logger.debug(
                f"_sync_checks_to_tab_context: {synced} atributos sincronizados para ctx"
            )
    ...
    def _debug_print_context_state(self, label: str):
        ...
        Util para diagnosticar bugs de estado "stale" apos troca de abas.
        """
        ...
        try:
            ...
            for attr in key_checks:
                ...
                valid = "N/A"
                if val:
                    # Verificar quantos sao widgets válidos
            ...
            for i, ctx in enumerate(contexts):
                ...
                for attr in key_checks:
                    ...
                    count = len(val) if val else 0
                    # Verificar se referência e a mesma que self
    ...
    def _schedule_adv_options_refresh(self):
        ...
        try:
            ...
            logger.error(f"Erro ao agendar refresh de filtros via QTimer: {e}")
            self._adv_options_scheduled = False
            try:
                self._run_adv_options_refresh()
            except Exception as e2:
                logger.error(
                    f"Erro ao executar refresh de filtros (fallback imediato): {e2}"
                )
    ...
    def _on_search_text_changed(self, text: str):
        """
        Handler para mudanca de texto no campo de busca.
        Usa debounce timer para evitar filtragem excessiva durante digitacao.
        """
        try:
    ...
    def _on_tab_changed(self, index: int) -> None:
        ...
        # CORRECAO LAYOUTREFACTOR #3: Restaurar checkboxes de derivadas
            ...
            if self._is_full_data_loaded:
                ...
                if hasattr(self, "_refresh_advanced_filter_options"):
                    try:
                        ...
                        logger.warning(
                            f"Erro ao atualizar filtros na troca de aba: {e}"
                        )
                ...
                # Forcar carregamento completo imediatamente
                logger.info("Usuário rápido detectado - forcando carregamento completo")
        ...
        # DEBUG-UI: Estado APOS a troca de aba
    ...
    def _make_multiselect_box(
        ...
        try:
            ...
            logger.error(f"Erro ao setar largura maxima do botao multiselect: {e}")
        try:
            button.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        except Exception as e:
            logger.error(f"Erro ao setar policy expansion do botao multiselect: {e}")
        menu = QMenu(button)
        try:
            menu.setMaximumHeight(360)
            # BUGFIX 2026-01-08: Prevenir fechamento automatico ao clicar
            menu.setWindowFlags(menu.windowFlags() | Qt.WindowType.WindowStaysOnTopHint)
        except Exception as e:
            logger.error(f"Erro ao configurar flags do menu: {e}")
        ...
        try:
            ...
            logger.error(f"Erro ao setar policy expansion do box multiselect: {e}")
        exclude = None
        if with_exclude:
            ...
            try:
                ...
                logger.error(f"Erro ao ocultar exclude checkbox: {e}")
        ...
        def _show_menu():
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Erro ao calcular posicao do menu: {e}")
                logger.debug(f"[MENU] Executando menu.exec() em pos={pos}")
                menu.exec(pos)
                logger.debug(f"[MENU] Menu fechado")
                return
            except Exception as e:
                logger.error(f"Erro ao exibir menu multiselect: {e}")
    ...
    def _update_multiselect_button(
        ...
        if not _is_widget_valid(button):
            return
        logger.debug(
            f"[BTN] Atualizando button, checks={len(checks or [])}, exclude={len(exclude_checks or [])}"
        )
        selected = []
        for cb in checks or []:
            try:
        ...
        try:
            ...
            if total == 0:
                ...
                if not getattr(self, "_is_full_data_loaded", False):
                    button.setStyleSheet(
                        "color: #666; background-color: #f9f9f9; font-style: italic;"
                    )
                else:
            ...
            if selected or excluded:
                ...
                button.setToolTip(
                    placeholder if total > 0 else "Nenhum dado disponivel"
                )
        except Exception as e:
            logger.error(f"Erro final em _update_multiselect_button: {e}")
    ...
    def _rebuild_multiselect_menu(
        ...
        if not _is_widget_valid(button) or not _is_widget_valid(menu):
            logger.debug(
                "_rebuild_multiselect_menu: button ou menu invalido/deletado - ignorando"
            )
            return [], []
        ...
        try:
            ...
            logger.error(f"Erro ao obter filter_name em _rebuild_multiselect_menu: {e}")
        ...
        try:
            ...
            min_width = max(
                int(getattr(button, "width", lambda: 0)() or 0),
                min(360, max(160, computed)),
            )
            menu.setMinimumWidth(min_width)
        except Exception as e:
            logger.error(f"Erro ao calcular largura minima do menu: {e}")
        ...
        try:
            ...
            logger.error(f"Erro ao alinhar grid: {e}")
        ...
        if filter_name:
            ...
            try:
                ...
                logger.error(f"Erro ao definir estilo do label_filter: {e}")
            ...
            if exclude_selected_set is not None:
                ...
                try:
                    ...
                    label_inc.setStyleSheet(
                        "font-size: 10px; color: #888; border: 1px solid #aaa; border-radius: 2px; padding: 1px 3px;"
                    )
                    ...
                    logger.error(
                        f"Erro ao definir estilo dos labels header (inc/exc): {e}"
                    )
                grid.addWidget(
        ...
        if total_items > MAX_MENU_ITEMS:
            ...
            try:
                display_values.sort(
                    key=lambda x: str(
                        x[1] if isinstance(x, (tuple, list)) else x
                    ).casefold()
                )
            except:
                pass
        ...
        for val in display_values:
            ...
            cb_value = (
                val[0] if isinstance(val, (list, tuple)) and len(val) > 0 else val
            )
            ...
            try:
                ...
                logger.error(f"Erro ao definir estilo label item: {e}")
            ...
            # BUGFIX 2026-01-08: Prevenir loops de sinais em mutuamente exclusivos
            if exclude_cb is not None:
                ...
                try:
                    ...
                    logger.error(f"Erro ao conectar sinais toggle exclusivos: {e}")
            if on_toggle is not None:
                try:
                    include_cb.toggled.connect(on_toggle)
                except Exception as e:
                    logger.error(f"Erro ao conectar on_toggle: {e}")
        ...
        if exclude_selected_set is not None:
            ...
            try:
                ...
                logger.error(f"Erro ao definir estilo labels select all: {e}")

            grid.addWidget(label_select, row_idx, 0)
            grid.addWidget(
                select_all_include, row_idx, 1, alignment=Qt.AlignmentFlag.AlignHCenter
            )
            grid.addWidget(
                select_all_exclude, row_idx, 2, alignment=Qt.AlignmentFlag.AlignHCenter
            )
            row_idx += 1

            grid.addWidget(label_deselect, row_idx, 0)
            grid.addWidget(
                deselect_all_include,
                row_idx,
                1,
                alignment=Qt.AlignmentFlag.AlignHCenter,
            )
            grid.addWidget(
                deselect_all_exclude,
                row_idx,
                2,
                alignment=Qt.AlignmentFlag.AlignHCenter,
            )
        ...
        try:
            ...
            logger.error(f"Erro ao definir alinhamento do scroll: {e}")
        try:
            ...
            container.setStyleSheet(
                f"QWidget {{ background: {bg}; }} QLabel {{ font-size: 11px; }}"
            )
            scroll.setStyleSheet(f"QScrollArea {{ border: 1px solid {border}; }}")
        except Exception as e:
            logger.error(f"Erro ao definir estilo do container: {e}")
        try:
            scroll.setFixedHeight(320)
        except Exception as e:
            logger.error(f"Erro ao definir altura fixa do scroll: {e}")
        scroll_act = QWidgetAction(menu)
        scroll_act.setDefaultWidget(scroll)
        try:
            menu.addAction(scroll_act)
        except Exception as e:
            logger.error(f"Erro ao adicionar action ao menu: {e}")
        ...
        if exclude_selected_set is not None:
            ...
            try:
                select_all_include.toggled.connect(
                    lambda checked: _select_all_include() if checked else None
                )
                deselect_all_include.toggled.connect(
                    lambda checked: _deselect_all_include() if checked else None
                )
                select_all_exclude.toggled.connect(
                    lambda checked: _select_all_exclude() if checked else None
                )
                deselect_all_exclude.toggled.connect(
                    lambda checked: _deselect_all_exclude() if checked else None
                )
            except Exception as e:
                logger.error(f"Erro ao conectar sinais de selecao: {e}")
        ...
        if on_apply is not None:
            ...
            try:
                ...
                logger.error(f"Erro conectando cancel_btn: {e}")
            try:
                ...
                logger.error(f"Erro conectando ok_btn: {e}")
            ok_row = QWidget()
            ...
            try:
                ...
                logger.error(f"Erro addAction ok_act: {e}")
    ...
    def _checkbox_value(self, checkbox) -> str:
        ...
        try:
            ...
            logger.error(f"Erro ao obter texto do checkbox: {e}")
        try:
            value = checkbox.property("value")
            if value is not None:
                return str(value)
        except Exception as e:
            logger.error(f"Erro ao obter propriedade value do checkbox: {e}")
        return ""
    ...
    def _build_advanced_filters_panel(self):
        ...
        # LAYOUTREFACTOR #4: Renomeacao para maior clareza
        div_box, div_button, div_menu, div_exclude = self._make_multiselect_box(
            "Divisao Emissora"
        )
        ...
        year_execucao_box, year_execucao_button, year_execucao_menu, _ = (
            self._make_multiselect_box("Ano Execucao", with_exclude=False)
        )
        ...
        deriv_layout = QVBoxLayout(deriv_box)  # MUDANCA: Vertical em vez de horizontal
        deriv_layout.setContentsMargins(
            4, 4, 4, 4
        )  # Margens maiores para melhor espacamento
        deriv_layout.setSpacing(4)  # Espacamento vertical entre checkboxes

        # Checkboxes (mantem as 3 existentes)
        deriv_has = QCheckBox("Possui Derivada(s)")  # Antes: "Tem"
        deriv_all_ste = QCheckBox("Derivada Concluida (STE)")  # Antes: "STE"
        deriv_is = QCheckBox("Status: SSA Derivada")  # Antes: "Sou Derivada"

        # Callbacks (mantem iguais)
        try:
            deriv_has.toggled.connect(
                lambda checked: self._on_derivada_has_toggled(checked)
            )
            deriv_all_ste.toggled.connect(
                lambda checked: self._on_derivada_all_ste_toggled(checked)
            )
        except Exception as e:
        ...
        # NOTA: Botao "Derivadas (n)" agora e widget separado no grid (ver abaixo)

        # GRIDREFACTOR V2: Botao Derivadas (n) como widget separado no grid
        ...
        # Armazenar referência para atualizacao do contador
        ...
        try:
            ...
            logger.error(f"Erro config week_emissao_start: {e}")
        ...
        try:
            ...
            logger.error(f"Erro config week_emissao_end: {e}")
        ...
        try:
            ...
            logger.error(f"Erro config week_exec_start: {e}")
        ...
        try:
            ...
            logger.error(f"Erro config week_exec_end: {e}")
        ...
        # GRIDREFACTOR V2: Renomeacões "Resp" → "Responsavel"
        ...
        exec_resp_box, exec_resp_button, exec_resp_menu, exec_resp_exclude = (
            self._make_multiselect_box("Responsavel Execucao")
        )
        ...
        data_exata_edit.setMaximumDate(
            QDate.currentDate().addDays(365 * 2)
        )  # 2 anos no futuro
        data_exata_edit.setDate(
            data_exata_edit.minimumDate()
        )  # Seta para minimum = mostra special text
        try:
        ...
        main_layout.addWidget(
            macro_panel, 0
        )  # Col 0: Macros (sem stretch, largura fixa)
        main_layout.addWidget(
            setores_col_widget, 1
        )  # Col 1: Setores/Divisões (stretch=1)
        main_layout.addWidget(
            status_col_widget, 1
        )  # Col 2: Status/Localização (stretch=1)
        main_layout.addWidget(
            data_reprog_col_widget, 1
        )  # Col 3: Data+Reprog+Resp (stretch=1)
        main_layout.addWidget(
            derivadas_resp_col_widget, 1
        )  # Col 4: Derivadas+Resp (stretch=1)
        ...
        if not hasattr(self, "_adv_filters_debounce_timer"):
            ...
            self._adv_filters_debounce_timer.timeout.connect(
    ...
    def _on_derivada_has_toggled(self, checked: bool):
        """Quando 'Tem' e desmarcado, 'STE' tambem deve ser desmarcado."""
        if not checked:
            try:
                if (
                    hasattr(self, "adv_derivada_all_ste")
                    and self.adv_derivada_all_ste.isChecked()
                ):
                    self.adv_derivada_all_ste.blockSignals(True)
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if (
                    numero
                    and derivada_de
                    and derivada_de.lower() not in ("nan", "none", "")
                ):
            ...
            try:
                ...
                logger.error(f"Erro style sheet text_edit derivadas: {e}")
    ...
    def _update_derivadas_button_state(self):
        ...
        try:
            ...
            # CORRECAO PREGOLDRELEASE #6: Mostrar contagem de derivadas no botao
            if has_derivadas:
                count = len(
                    df[derivada_col]
                    .dropna()
    ...
    def _update_derivadas_counter(self):
        """Atualiza contador de derivadas no botao 'Derivadas (n)'."""
        try:
    ...
    def _save_advanced_filters_default(self):
        ...
        try:
            ...
            gui_settings["advanced_filters_default"] = dict(
                self._advanced_filters or {}
            )
            self._persist_gui_preferences()
        except Exception as e:
    ...
    def _on_edit_macro_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro", f"Erro ao editar macro: {e}")
    ...
    def _on_save_macro_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Erro", f"Erro ao salvar macro: {e}")
    ...
    def _connect_advanced_filters_auto_apply(self):
        ...
        try:
            ...
            multiselect_buttons = [
                ...
                (
                    "adv_responsavel_solicitante_menu",
                    self.adv_responsavel_solicitante_menu,
                ),
                (
                    "adv_responsavel_programacao_menu",
                    self.adv_responsavel_programacao_menu,
                ),
            ...
            if (
                hasattr(self, "adv_data_exata_edit")
                and self.adv_data_exata_edit is not None
            ):
                try:
                    self.adv_data_exata_edit.dateChanged.connect(
                        self._apply_advanced_filters_from_ui
                    )
                except Exception as e:
            ...
            logger.info(
                "Aplicação automática de filtros avançados conectada com sucesso"
            )
    ...
    def _on_adv_sector_selection_changed(self, *_):
        ...
        try:
            ...
            logger.error(f"Erro ao aplicar divisao a setores: {e}")
        try:
            self._update_multiselect_button(
            ...
            logger.error(f"Erro ao atualizar botao executor: {e}")
        try:
            self._update_multiselect_button(
            ...
            logger.error(f"Erro ao atualizar botao emissor: {e}")
        try:
            self._update_multiselect_button(
            ...
            logger.error(f"Erro ao atualizar botao divisao: {e}")
        try:
            self._refresh_responsavel_options()
        except Exception as e:
            logger.error(f"Erro ao atualizar opcoes de responsavel: {e}")

    def _on_adv_sector_exclude_changed(self, *_):
        """Atualiza filtros de exclusao de setor com debouncing."""
        try:
            self._update_multiselect_button(
                self.adv_executor_button,
                self.adv_executor_checks,
            ...
            logger.error(f"Erro ao atualizar botao executor (exclude): {e}")
        try:
            self._update_multiselect_button(
                self.adv_emissor_button,
                self.adv_emissor_checks,
            ...
            logger.error(f"Erro ao atualizar botao emissor (exclude): {e}")
        try:
            self._update_multiselect_button(
                self.adv_divisao_button,
                self.adv_divisao_checks,
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Erro no fallback de refresh responsavel: {e2}")
    ...
    def _collect_divisao_setores(self, divisao_values):
        ...
        for div in divisao_values or []:
            try:
                ...
                logger.error(f"Erro ao coletar setores da divisao {div}: {e}")
        return setores
    ...
    def _sort_responsavel_values(self, df_subset, values, resp_col: str):
        ...
        for col in sector_cols:
            try:
                ...
                logger.debug(f"Erro ao obter pares responsavel/setor para {col}: {e}")
                continue
        ...
        def _best_sector(person):
            ...
            if not counts:
                return ""
            return max(
                counts.items(), key=lambda t: (t[1], -len(t[0]), t[0].casefold())
            )[0]
    ...
    def _apply_divisao_to_setor_checks(self):
        ...
        """
        selected_div = self._get_checked_values(
        ...
        try:
            ...
            for cb in getattr(self, "adv_emissor_checks", None) or []:
                try:
                    ...
                    logger.error(f"Erro ao marcar emissor check: {e}")
        finally:
            self._adv_sector_syncing = False
    ...
    def _refresh_responsavel_options(self):
        if self.df_completo is None or self.df_completo.empty:
            return
        exec_values = self._get_checked_values(
            getattr(self, "adv_executor_checks", None)
        )
        emis_values = self._get_checked_values(
        ...
        div_excluded = self._get_checked_values(
        ...
        def _set_enabled(widget, enabled):
            ...
            try:
                ...
                logger.error(f"Erro ao habilitar widget {widget}: {e}")
        ...
        def _set_visible(widget, visible):
            ...
            try:
                ...
                logger.error(f"Erro ao tornar visivel widget {widget}: {e}")
        ...
        def _apply_sector_subset(frame):
            ...
            if emis_col in subset.columns and selected_emis_excluded:
                subset = subset[
                    ~subset[emis_col].astype(str).isin(selected_emis_excluded)
                ]
            return subset
        ...
        def _unique_sorted(col):
            try:
                ...
                logger.error(f"Erro ao ordenar valores unicos da coluna {col}: {e}")
                return []
        ...
        for col, prefix in resp_cols:
            ...
            try:
                ...
                logger.debug(f"Falha ao ordenar responsaveis: {e}")
    ...
    def _populate_responsavel_filters(self):
        ...
        try:
            include_checks, _ = self._rebuild_multiselect_menu(
                ...
                reprog_values,
                set(
                    (self._advanced_filters or {}).get("num_reprogramacoes_values")
                    or []
                ),
                lambda *_: self._update_multiselect_button(
                    getattr(self, "adv_reprog_button", None),
            ...
            # CORRECAO PREGOLDRELEASE #1: Atualizar texto do botao apos rebuild
            self._update_multiselect_button(
                getattr(self, "adv_reprog_button", None),
                getattr(self, "adv_reprog_checks", None),
            )
            ...
            try:
                ...
                logger.debug(f"Erro ao setar modo reprogramacoes: {e}")
        except Exception as e:
            logger.error(f"Erro ao reconstruir menu reprogramacoes: {e}")
            self.adv_reprog_checks = []

        # SSAs Derivadas Especificas (novo filtro granular)
        ...
        if not derivadas_numbers:
            ...
            try:
                ...
                logger.debug(f"Erro ao extrair derivadas unicas para filtro adv: {e}")
    ...
    def _clear_advanced_filters(self):
        ...
        # CORRECAO LAYOUTREFACTOR #2: Desmarcar checkboxes de derivadas (com blockSignals)
        if hasattr(self, "adv_derivada_has") and self.adv_derivada_has:
            self.adv_derivada_has.blockSignals(True)
            self.adv_derivada_has.setChecked(False)
            self.adv_derivada_has.blockSignals(False)
        ...
        try:
            ...
            logger.error(f"Erro ao sincronizar UI apos limpar filtros: {e}")
        try:
            self._refresh_after_filter_change()
        except Exception as e:
            logger.error(f"Erro ao atualizar apos limpar filtros: {e}")
    ...
    def _apply_advanced_filters_from_ui(self, store_only: bool = False):
        if not store_only:
            try:
                ...
                logger.error(f"Erro ao salvar estado do filtro: {e}")
        data = {}
        try:
            data["setor_executor"] = self._get_checked_values(
                getattr(self, "adv_executor_checks", None)
            )
        except Exception as e:
            logger.error(f"Erro ao obter executor checks: {e}")
            data["setor_executor"] = []
        try:
            ...
            logger.error(f"Erro ao obter executor exclude checks: {e}")
            data["setor_executor_exclude_values"] = []
        try:
            data["setor_emissor"] = self._get_checked_values(
                getattr(self, "adv_emissor_checks", None)
            )
        except Exception as e:
            logger.error(f"Erro ao obter emissor checks: {e}")
            data["setor_emissor"] = []
        try:
            ...
            logger.error(f"Erro ao obter emissor exclude checks: {e}")
            data["setor_emissor_exclude_values"] = []
        try:
            data["divisao"] = self._get_checked_values(
                getattr(self, "adv_divisao_checks", None)
            )
        except Exception as e:
            logger.error(f"Erro ao obter divisao checks: {e}")
            data["divisao"] = []
        try:
            ...
            logger.error(f"Erro ao obter divisao exclude checks: {e}")
            data["divisao_exclude_values"] = []
        try:
            data["situacao"] = self._get_checked_values(
                getattr(self, "adv_status_checks", None)
            )
        except Exception as e:
            logger.error(f"Erro ao obter status checks: {e}")
            data["situacao"] = []
        try:
            ...
            logger.error(f"Erro ao obter status exclude checks: {e}")
        ...
        try:
            ...
            logger.error(f"Erro ao obter ano_emissao checks: {e}")
        ...
        try:
            ...
            logger.error(f"Erro ao obter ano_emissao exclude checks: {e}")
        ...
        try:
            ...
            logger.error(f"Erro ao obter ano_execucao checks: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro ano_execucao exclude: {e}")
            data["ano_execucao_exclude_values"] = []
        try:
            data["semana_emissao_inicio"] = self._parse_week(
                self.adv_week_emissao_start.text()
            )
            data["semana_emissao_fim"] = self._parse_week(
                self.adv_week_emissao_end.text()
            )
        except Exception as e:
            logger.error(f"Erro filtro semana_emissao: {e}")
            data["semana_emissao_inicio"] = None
            data["semana_emissao_fim"] = None
        try:
            data["semana_execucao_inicio"] = self._parse_week(
                self.adv_week_execucao_start.text()
            )
            data["semana_execucao_fim"] = self._parse_week(
                self.adv_week_execucao_end.text()
            )
        except Exception as e:
            logger.error(f"Erro filtro semana_execucao: {e}")
        ...
        try:
            data["derivada_has"] = bool(
                getattr(self, "adv_derivada_has", None).isChecked()
            )
        except Exception as e:
            logger.error(f"Erro filtro derivada_has: {e}")
            data["derivada_has"] = False
        try:
            data["derivada_all_ste"] = bool(
                getattr(self, "adv_derivada_all_ste", None).isChecked()
            )
        except Exception as e:
            logger.error(f"Erro filtro derivada_all_ste: {e}")
            data["derivada_all_ste"] = False
        if data.get("derivada_all_ste"):
            data["derivada_has"] = True
        try:
            data["derivada_is"] = bool(
                getattr(self, "adv_derivada_is", None).isChecked()
            )
        except Exception as e:
            logger.error(f"Erro filtro derivada_is: {e}")
            data["derivada_is"] = False
        # derivadas_especificas_values removido - botao Especificas agora e apenas visualizacao
        try:
            data["solicitante"] = self._get_checked_values(
                getattr(self, "adv_responsavel_solicitante_checks", None)
            )
        except Exception as e:
            logger.error(f"Erro filtro solicitante: {e}")
            data["solicitante"] = []
        try:
            ...
            logger.error(f"Erro filtro solicitante exclude: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro resp prog: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro resp prog exclude: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro resp exec: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro resp exec exclude: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro resp emissor: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro resp emissor exclude: {e}")
            data["responsavel_emissor_exclude_values"] = []
        try:
            data["num_reprogramacoes_values"] = self._get_checked_values(
                getattr(self, "adv_reprog_checks", None)
            )
        except Exception as e:
            logger.error(f"Erro filtro reprogramacoes values: {e}")
            data["num_reprogramacoes_values"] = []
        try:
            mode_idx = getattr(self, "adv_reprog_mode", None).currentIndex()
            data["num_reprogramacoes_mode"] = getattr(
                self, "adv_reprog_mode", None
            ).itemData(mode_idx)
        except Exception as e:
            logger.error(f"Erro filtro reprogramacoes mode: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro prio emissao: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro prio emissao exclude: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro prio plan: {e}")
        ...
        try:
            ...
            logger.error(f"Erro ao atualizar pos filtro: {e}")
        ...
        if isinstance(source, list):
            for child in source:
                try:
                    ...
                    logger.error(f"Erro ao ler valor do checkbox (lista): {e}")
            return values
        if hasattr(source, "findChildren"):
            try:
                children = source.findChildren(QCheckBox)
            except Exception as e:
                logger.error(f"Erro ao buscar checkboxes filhos: {e}")
                children = []
            for child in children:
                try:
                    ...
                    logger.error(f"Erro ao ler valor do checkbox (widget): {e}")
        return values
    ...
    def _sync_advanced_filter_ui(self):
        ...
        # Se checks estiverem vazios, forcar rebuild de menus
        # CORRECAO LAYOUTREFACTOR #1: Adicionar protecao contra loop infinito
        if not getattr(self, "adv_executor_checks", None):
            # Verificar se já está sincronizando para evitar recursao infinita
            if getattr(self, "_syncing_advanced_ui", False):
                logger.debug("_syncing_advanced_ui já em execucao - evitando recursao")
                return

            self._syncing_advanced_ui = True
            logger.debug(
                "adv_executor_checks vazio - forcando rebuild de menus avancados"
            )
            try:
                if hasattr(self, "_refresh_advanced_filter_options"):
                    self._refresh_advanced_filter_options()
        ...
        # FASE5: LOCALIZAÇÃO - Sincronização de UI
        self._sync_multiselect_checks(
            getattr(self, "adv_localizacao_button", None),
            getattr(self, "adv_localizacao_checks", None),
            data.get("localizacao_values"),
            getattr(self, "adv_localizacao_exclude_checks", None),
            data.get("localizacao_exclude_values"),
        )
        # FASE5: DIVISÃO EXECUTORA - Sincronização de UI
        self._sync_multiselect_checks(
            getattr(self, "adv_divisao_executora_button", None),
            getattr(self, "adv_divisao_executora_checks", None),
            data.get("divisao_executora_values"),
            getattr(self, "adv_divisao_executora_exclude_checks", None),
            data.get("divisao_executora_exclude_values"),
        )
        self._sync_multiselect_checks(
            getattr(self, "adv_status_button", None),
            getattr(self, "adv_status_checks", None),
        ...
        try:
            ...
            logger.error(f"Erro ao sincronizar ui ano emissao: {e}")
        try:
            ...
            logger.error(f"Erro ao sincronizar ui ano execucao: {e}")
        try:
            self.adv_week_emissao_start.setText(
                ""
                if data.get("semana_emissao_inicio") is None
                else str(data.get("semana_emissao_inicio"))
            )
            self.adv_week_emissao_end.setText(
                ""
                if data.get("semana_emissao_fim") is None
                else str(data.get("semana_emissao_fim"))
            )
            self.adv_week_execucao_start.setText(
                ""
                if data.get("semana_execucao_inicio") is None
                else str(data.get("semana_execucao_inicio"))
            )
            self.adv_week_execucao_end.setText(
                ""
                if data.get("semana_execucao_fim") is None
                else str(data.get("semana_execucao_fim"))
            )
        except Exception as e:
        ...
        try:
            ...
            try:
                if (
                    hasattr(self, "adv_macro_combo")
                    and self.adv_macro_combo is not None
                ):
    ...
    def _refresh_advanced_filter_options(self):
        ...
        df = self.df_completo
        logger.debug(
            f"_refresh_advanced_filter_options: iniciando com {len(df)} registros"
        )
        filters = self._advanced_filters or {}
        apply_cb = lambda: self._apply_advanced_filters_from_ui()
        ...
        def _unique_sorted(col):
            try:
                ...
                logger.error(f"Erro em _unique_sorted({col}): {e}")
                return []
        ...
        def _sort_sector_values(values):
            try:
                ...
                logger.error(f"Erro em _sort_sector_values: {e}")
        ...
        # Popula cache se necessário (bloco unico consolidado) - CORRIGIDO: removida duplicacao
        if cache.get("exec_vals") is None:
            ...
            def _collect_years_from_dates(series):
                """Extrai anos de datas usando operacões vetorizadas (otimizado)."""
                try:
                    ...
                    logger.error(f"Erro em _collect_years_from_dates: {e}")
                    return []
            ...
            def _collect_years_from_weeks(series):
                ...
                try:
                    ...
                    logger.error(f"Erro em _collect_years_from_weeks: {e}")
                    return []
            ...
            if "num_reprogramacoes" in df.columns:
                try:
                    ...
                    logger.error(f"Erro em cache reprog_vals: {e}")
                    cache["reprog_vals"] = []
            else:
                cache["reprog_vals"] = []
            self._adv_values_cache = cache
            logger.debug(
                f"_refresh_advanced_filter_options: cache populado - exec={len(cache.get('exec_vals', []))}, emis={len(cache.get('emis_vals', []))}, status={len(cache.get('status_vals', []))}"
            )
            ...
            year_include, year_exclude = self._rebuild_multiselect_menu(
                ...
                lambda *_: self._update_multiselect_button(
                    ...
                    exclude_checks=getattr(
                        self, "adv_year_emissao_exclude_checks", None
                    ),
                ),
                apply_cb,
                exc_set,
        ...
        if hasattr(self, "adv_year_execucao_menu"):
            ...
            # FUNCIONALIDADE PREGOLDRELEASE: Auto-marcar STE quando Ano Execucao selecionado
            def _on_ano_execucao_toggle(*_):
                self._update_multiselect_button(
                    self.adv_year_execucao_button,
                    getattr(self, "adv_year_execucao_checks", None),
                    exclude_checks=getattr(
                        self, "adv_year_execucao_exclude_checks", None
                    ),
                )
        ...
        # Atualiza texto dos botões multiselect apos popular checkboxes
        # CORRECAO 2026-01-15: Botões ficavam com "Sem dados" mesmo apos popular menus
        ...
        if hasattr(self, "adv_responsavel_solicitante_button"):
            self._update_multiselect_button(
                ...
                getattr(self, "adv_responsavel_solicitante_checks", None),
                exclude_checks=getattr(
                    self, "adv_responsavel_solicitante_exclude_checks", None
                ),
            )
        if hasattr(self, "adv_responsavel_programacao_button"):
            self._update_multiselect_button(
                ...
                getattr(self, "adv_responsavel_programacao_checks", None),
                exclude_checks=getattr(
                    self, "adv_responsavel_programacao_exclude_checks", None
                ),
            )
        if hasattr(self, "adv_responsavel_execucao_button"):
            self._update_multiselect_button(
                ...
                getattr(self, "adv_responsavel_execucao_checks", None),
                exclude_checks=getattr(
                    self, "adv_responsavel_execucao_exclude_checks", None
                ),
            )
        if hasattr(self, "adv_responsavel_emissor_button"):
            self._update_multiselect_button(
                ...
                getattr(self, "adv_responsavel_emissor_checks", None),
                exclude_checks=getattr(
                    self, "adv_responsavel_emissor_exclude_checks", None
                ),
            )

        # BUGFIX 2026-01-19: Sincronizar checkbox lists de volta para _tab_contexts
        # Apos rebuild, os novos checkboxes estao em self.* mas o ctx em _tab_contexts
        ...
        # DEBUG-UI: Estado apos rebuild dos menus de filtros avancados
        ...
        try:
            ...
            logger.debug(f"Erro ao logar metricas de refresh: {e}")
    ...
    def _apply_advanced_filters(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def _apply_in(col, values, exclude_values=None):
            ...
            try:
                ...
                logger.error(f"Erro em _apply_in({col}): {e}")
        ...
        if "setor_executor" in df.columns:
            ...
            try:
                ...
                logger.error(f"Erro filtro setor_executor: {e}")
        ...
        try:
            ...
            logger.error(f"Erro filtro reprogramacoes: {e}")
        ...
        if emissao_inc or emissao_exc:
            if "data_cadastro" in df.columns:
                try:
                    ...
                    logger.error(f"Erro filtro data_cadastro (ano): {e}")
            elif "semana_cadastro" in df.columns:
                try:
                    nums = pd.to_numeric(df["semana_cadastro"], errors="coerce").astype(
                        "Int64"
                    )
                    ...
                    logger.error(f"Erro filtro semana_cadastro (ano): {e}")
        ...
        if "semana_executada" in df.columns and (execucao_inc or execucao_exc):
            try:
                ...
                logger.error(f"Erro filtro semana_executada (ano): {e}")
        ...
        def _apply_week_range(col, start_key, end_key, exclude_key):
            ...
            try:
                ...
                logger.error(f"Erro filtro _apply_week_range({col}): {e}")
        ...
        if "derivada_de" in df.columns:
            ...
            if (derivada_has or derivada_all_ste) and "numero_ssa" in df.columns:
                try:
                    ...
                    logger.error(f"Erro ao obter origens derivadas: {e}")
                    origins = set()
                if derivada_all_ste and "situacao" in df.columns:
                    try:
                        ...
                        logger.error(f"Erro ao filtrar derivadas STE: {e}")
                        origins = set()
                if origins:
                    try:
    ...
    def get_ssa_data_provider(self, numero_ssa: str):
        """
        Provedor de dados para o DetailsTabWidget.
        Retorna Series com dados da SSA ou None.
        """
        if self.df_completo is None or self.df_completo.empty:
            return None

        # Otimizacao: Usa coluna pre-calculada se disponivel
        if "_norm_ssa" in self.df_completo.columns:
        ...
        if mask.any():
            return self.df_completo[mask].iloc[0]
        return None
    ...
    def load_data(self):
        ...
        if not os.path.exists(DB_PATH):
            ...
            QMessageBox.warning(
                self,
                "Erro",
                f"Banco de dados '{DB_PATH}' nao encontrado. Execute o programa principal primeiro.",
            )
            return
        ...
        try:
            self.data_loader_thread.finished.connect(
                self.data_loader_thread.deleteLater
            )
        except Exception as e:
            logger.debug(f"Erro ao conectar deleteLater no loader: {e}")
        self.data_loader_thread.start()

    def on_data_loaded(self, df: pd.DataFrame):
        self.df_completo = df.copy()

        # Otimizacao: Pre-calcula coluna de SSA normalizada para busca rapidae
        # Evita recalcular em detalhes_tab_manager a cada abertura
        if "numero_ssa" in self.df_completo.columns:
            try:
                # Usa map que e mais rapido que apply para series
                self.df_completo["_norm_ssa"] = self.df_completo["numero_ssa"].map(
                    normalize_ssa_value
                )
            except Exception as e:
                logger.error(f"Erro ao criar indice normalizado de SSA: {e}")
        ...
        try:
            ...
            if "numero_ssa" in base.columns:
                ssa_str = (
                    base["numero_ssa"].astype(str).str.replace(r"\D", "", regex=True)
                )
        ...
        try:
            ...
            logger.warning("Falha ao agendar atualizacao de filtros avancados: %s", e)
        try:
            self._update_derivadas_button_state()
        except Exception as e:
            logger.error(f"Erro ao atualizar botao derivadas: {e}")
        profile_hint = (
            f" (perfil: {self.current_filter_profile})"
    ...
    # =============================================================================
    # LAZY LOADING: Metodos de gerenciamento de buffer e cache
    # =============================================================================
    ...
    def _clear_filters_dirty(self) -> None:
        """Limpa flag de filtros sujos apos atualizacao de cache."""
        self._filters_dirty = False
        logger.debug("Flag de filtros dirty limpa")

    def _populate_filter_buffer(self) -> None:
        """
        Popula cache de filtros com valores unicos do buffer inicial (50 SSAs).
        Usado para popular dropdowns rapidamente na inicializacao.
        """
        ...
        logger.debug(
            f"Populando buffer de filtros com {len(self.df_completo)} registros"
        )
        ...
        for filter_key, column_name in filter_mapping.items():
            if column_name in self.df_completo.columns:
                ...
                logger.debug(
                    f"Coluna {column_name} nao encontrada para filtro {filter_key}"
                )
                self._filter_buffer_cache[filter_key] = set()

    def _populate_full_filter_cache(self, filter_type: str) -> None:
        """
        Carrega valores completos para um tipo de filtro especifico sob demanda.
        ...
        if not self._is_full_data_loaded:
            logger.info(
                f"_populate_full_filter_cache({filter_type}): Dados completos ainda nao carregados"
            )
            return
        ...
        if column_name not in self.df_completo.columns:
            logger.warning(
                f"Coluna {column_name} nao encontrada para filtro {filter_type}"
            )
            return
        ...
        try:
            ...
            logger.debug(
                f"Cache completo {filter_type}: {old_count} -> {new_count} valores"
            )
    ...
    def _on_buffer_loaded(self, df: pd.DataFrame):
        """
        Callback quando buffer inicial (50 SSAs) e carregado.
        ...
        # Normalizacao (necessária para busca de SSA)
        if "numero_ssa" in self.df_completo.columns:
            try:
                self.df_completo["_norm_ssa"] = self.df_completo["numero_ssa"].map(
                    normalize_ssa_value
        ...
        # Remove apenas filtros de colunas que nao existem mais no novo dataset
        from collections import OrderedDict
        ...
        for col_key, filter_value in old_filters.items():
            if col_key in self.df_completo.columns:
                ...
                # Coluna nao existe mais - remover filtro
                removed_count += 1
                logger.warning(
                    f"Filtro de coluna removido (coluna nao existe): {col_key}"
                )

        if preserved_count > 0:
            logger.info(
                f"Filtros preservados: {preserved_count}, removidos: {removed_count}"
            )
        else:
        ...
        # LAZY LOADING: NÃO atualiza filtros avancados com buffer - aguarda dados completos
        # Os filtros serao atualizados corretamente em _on_full_data_loaded()
        # if hasattr(self, '_refresh_advanced_filter_options'):
        #     try:
        #         self._refresh_advanced_filter_options()
        #     except Exception as e:
        #         logger.warning(f"Erro ao atualizar opcões de filtros: {e}")
        ...
        if preserved_count > 0 and hasattr(self, "_build_column_filters_panel"):
            try:
                self._build_column_filters_panel()
                logger.debug("Painel de filtros reconstruido com filtros preservados")
            except Exception as e:
        ...
        # FASE 2: Inicia carregamento completo em background apos 1s
        # Delay de 1s para evitar conflitos com inicializacao
    ...
    def _start_full_data_load(self):
        ...
        # Evita iniciar carregamento multiplas vezes
        if self._full_data_loading_started or self._is_full_data_loaded:
            logger.debug("Carregamento completo já iniciado ou concluido - ignorando")
            return
        ...
        self._full_data_loader_thread.error_occurred.connect(
            self._on_full_data_load_error
        )
    ...
    def _on_full_data_loaded(self, df: pd.DataFrame):
        ...
        # Marca filtros como dirty para forcar atualizacao
        self._mark_filters_dirty()

        # Atualiza opcões de filtros avancados com dados completos
        # SEMPRE popula cache, independente da aba ativa
        if hasattr(self, "_refresh_advanced_filter_options"):
            try:
                self._refresh_advanced_filter_options()
                logger.debug("Filtros avancados atualizados com dados completos")
            except Exception as e:
                logger.warning(f"Erro ao atualizar opcões de filtros: {e}")
        ...
        # Limpa flag dirty apos atualizacao
        ...
        if hasattr(self, "_update_col_filter_indicator"):
            try:
                self._update_col_filter_indicator()
                logger.debug(
                    "Indicador de filtros atualizado apos carregamento completo"
                )
            except Exception as e:
                logger.warning(f"Erro ao atualizar indicador apos carregamento: {e}")

        logger.info("Lazy loading concluido com sucesso")
    ...
    def _on_full_data_load_error(self, error_msg: str):
        ...
        # Nao mostra erro ao usuário - buffer já está funcional
        # Apenas loga para debugging

    def _ensure_full_data_loaded(self, operation_name: str = "esta operacao") -> bool:
        """
        Garante que dados completos estejam carregados antes de executar operacao.

        Args:
            operation_name: Nome da operacao para mensagem ao usuário

        Returns:
            True se dados completos estao carregados, False caso contrário
        """
        if self._is_full_data_loaded:
            return True
        ...
        if self._full_data_loader_thread and self._full_data_loader_thread.isRunning():
            ...
            reply = QMessageBox.question(
                ...
                f"Dados completos ainda estao sendo carregados.\n\n"
        ...
        # Carregamento nao foi iniciado - inicia agora
        reply = QMessageBox.question(
            self,
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self._full_data_loader_thread.wait()  # Aguarda sincronamente
            return self._is_full_data_loaded
        ...
        try:
            ...
            if worker is not None:
                try:
                    ...
                    logger.debug(f"Erro ao parar worker antigo: {e}")
                try:
                    worker.deleteLater()
                except Exception:
                    pass  # deleteLater pode falhar se objeto ja deletado
        finally:
            self.data_loader_thread = None
    ...
    def on_header_clicked(self, logical_index: int):
        try:
            ...
            # Alterna direcao ao clicar na mesma coluna
            ...
            # Ordena resultado filtrado atual e reinicia paginacao
            try:
                ...
                logger.error(f"Erro ao ordenar df_exibido por {self.sort_column}: {e}")
            ...
            try:
                ...
                logger.error(f"Erro indicador visual sort: {e}")
        except Exception as e:
            logger.error(f"Erro em on_header_clicked: {e}")

    # --- Filtro por coluna via clique direito no cabecalho ---
    def show_header_context_menu(self, pos):
        try:
            ...
            menu = QMenu(self)
            full_name = DEFAULT_DISPLAY_MAPPINGS.get(
                col_name, self.internal_to_display.get(col_name, col_name)
            )
            ...
            def _apply():
                ...
                try:
                    ...
                    logger.error(f"Impossivel importar QInputDialog: {e}")
                    QInputDialog = None
                except Exception as e:
                    logger.error(f"Erro inesperado importando QInputDialog: {e}")
                    QInputDialog = None
                if QInputDialog:
                    ok = False
            ...
            logger.error(f"Erro show_header_context_menu: {e}")

    # Garante menu de contexto no cabecalho em qualquer tema/estilo
    def resizeEvent(self, event):
        """Detecta mudancas de tamanho da janela para adaptar layout."""
        try:
            super().resizeEvent(event)
        except Exception as e:
            logger.error(f"Erro em super().resizeEvent: {e}")
        ...
        try:
            ...
            if obj is header:
                ...
                if et == QEvent.Type.ContextMenu:
                    ...
                    return True
                # Qt6: MouseButtonPress com botcao direito
            ...
            logger.error(f"Erro em eventFilter header: {e}")
        return super().eventFilter(obj, event)

    # --- Helpers: painel e aplicacao dos filtros por coluna ---
    def toggle_theme_menu(self):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Erro ao setar WA_StyledBackground: {e}")
            ...
            logger.error(f"Erro ao estilizar QMenu tema: {e}")
        light_themes, dark_themes = self._get_theme_catalog()
        gui_settings = GUI_MAIN_PREFERENCES.get("gui_settings", {})
        theme_default = gui_settings.get("theme_default")
        current_theme = normalize_theme(
            getattr(self, "_current_theme", "") or theme_default or "gruvbox"
        )
        ...
        try:
            ...
            logger.error(f"Erro ao obter palette menu: {e}")
            wtxt = "#ffffff"
            win = "#000000"
        support_color = (
            roles.get("support_text_color") or roles.get("label_color") or wtxt
        )
        ...
        try:
            ...
            check_widget.setChecked(
                normalize_theme(theme_default or "") == current_theme
            )
            try:
                check_widget.setStyleSheet(f"color: {wtxt}; padding: 4px 10px;")
            except Exception as e:
                logger.debug(f"Style set failed on check_widget: {e}")

            def _toggle_default(checked):
                gui_settings = GUI_MAIN_PREFERENCES.setdefault("gui_settings", {})
                if checked:
                    active_theme = normalize_theme(
                        getattr(self, "_current_theme", "") or "gruvbox"
                    )
            ...
            logger.error(f"Erro QAction default theme: {e}")
            default_action = menu.addAction("Usar tema atual como padrao")
            if default_action is not None:
                try:
                    default_action.setCheckable(True)
                    default_action.setChecked(
                        normalize_theme(theme_default or "") == current_theme
                    )

                    def _toggle_default_action(checked):
                        gui_settings = GUI_MAIN_PREFERENCES.setdefault(
                            "gui_settings", {}
                        )
                        if checked:
                            active_theme = normalize_theme(
                    ...
                    logger.error(f"Erro fallback default theme action: {e2}")
        menu.addSeparator()
        ...
        def _add_label(text: str):
            try:
                ...
                try:
                    ...
                    logger.debug(f"Erro ao setar style label theme: {e}")
                action = QWidgetAction(menu)
                action.setDefaultWidget(label)
                menu.addAction(action)
            except Exception as e:
                logger.error(f"Erro _add_label: {e}")
                act = menu.addAction(text)
                if act is not None:
                    try:
                        act.setEnabled(False)
                    except Exception as e:
                        logger.debug(f"Falha ao desabilitar acao label: {e}")
                ...
                if act is not None:
                    ...
                    if trigger is not None:
                        try:
                            ...
                            logger.error(f"Erro connect theme trigger: {e}")
        ...
        try:
            ...
            logger.error(f"Erro min width menu: {e}")
        btn = self.sender()
        try:
            if btn is not None:
                menu.exec(btn.mapToGlobal(btn.rect().bottomLeft()))
        except Exception as e:
            logger.error(f"Erro exec menu: {e}")
    ...
    def apply_theme(self, name: str):
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Erro ao setar estilo Fusion: {e}")
            ...
            if app is not None:
                ...
                try:
                    ...
                    logger.warning(f"Erro ao aplicar QSS global: {e}")
            # Garante tambem na janela atual
            self.setPalette(pal)
        except Exception as e:
            logger.error(f"Erro principal apply_theme: {e}")
            pal = get_palette(normalized)
            self.setPalette(pal)
            ...
            try:
                ...
                if central is not None:
                    try:
                        ...
                        logger.debug(f"Erro limpar style central widget: {e}")
        ...
        try:
            ...
            for name in adv_buttons:
                ...
                if btn is not None:
                    try:
                        ...
                        logger.debug(f"Erro style adv button {name}: {e}")
            adv_line_edits = [
                "adv_week_emissao_start",
                "adv_week_emissao_end",
                "adv_week_execucao_start",
            ...
            for name in adv_line_edits:
                ...
                if widget is not None:
                    try:
                        ...
                        logger.debug(f"Erro style adv line edit {name}: {e}")
            ...
            if hasattr(self, "details_text"):
                if hasattr(self, "details_group"):
                    try:
                        ...
                        logger.debug(f"Erro ao ajustar fonte details em theme: {e}")
            ...
            if hasattr(self, "col_filters_hint"):
                self.col_filters_hint.setStyleSheet(
                    f"color:{support_color}; font-size: 11px;"
                )
        except Exception as e:
            logger.error(f"Erro em apply_theme sections 3-10: {e}")
    ...
    def _apply_macos_contrast(self, theme_name: str):
        ...
        try:
            ...
            logger.error(f"Erro _apply_macos_contrast: {e}")

    def on_columns_changed(self, new_columns):
        """Chamado quando a selecao de colunas muda."""
        ...
        if hasattr(self, "column_selector") and self.column_selector is not None:
            try:
                ...
                logger.error(f"Erro column_selector.set_selected_columns: {e}")
        # Reexibe a pagina atual com as novas colunas
        self.display_current_page(self.paginator.current_page)
        # Nota: Persistencia de preferencias removida para isolamento do CLI
        # As configuracões ficam no arquivo gui_main_preferences.json
    ...
    def display_current_page(self, page_number):
        ...
        # Congela redimensionamento automático durante a reconstrucao da tabela
        try:
            header = self.table_widget.horizontalHeader()
            header.setSectionResizeMode(QHeaderView.ResizeMode.Fixed)
        except Exception as e:
            logger.error(f"Erro header.setSectionResizeMode: {e}")
            header = None

        if self.df_para_tabela.empty:
            # Mesmo sem linhas, mantenha as colunas visiveis e larguras aplicadas
            self.table_widget.setRowCount(0)
            # Determina colunas válidas a partir de df_exibido (mesmo vazio, mantem schema)
            valid_cols = []
            try:
                ...
                logger.error(f"Erro ao determinar colunas validas: {e}")
                valid_cols = list(self.visible_columns)

            if not valid_cols:
                valid_cols = (
                    [c for c in self.default_columns if c in base_cols]
                    if base_cols
                    else list(self.visible_columns)
                )
            ...
            try:
                ...
                logger.error(f"Erro setHorizontalHeaderLabels: {e}")
            ...
            for i, col_name in enumerate(self._current_display_columns):
                ...
                try:
                    ...
                    logger.debug(f"Falha ao setar largura coluna {i}: {e}")
            ...
            try:
                ...
                logger.debug(f"Falha ao validar largura col 1: {e}")
            ...
            try:
                ...
                logger.error(f"Erro restaurar header mode: {e}")
            return
        ...
        if not cols_to_show:
            ...
            cols_to_show = [
                col
                for col in self.default_columns
                if col in self.df_para_tabela.columns
            ]
            if not cols_to_show:
        ...
        if "#" not in display_df.columns:
            display_df.insert(
                ...
                range(
                    ...
                    (self.paginator.current_page - 1) * self.paginator.page_size
                    + 1
                    + len(display_df),
                ),
            )
        ...
        if cached_formatted is None:
            try:
                ...
                logger.error(f"Erro ao formatar display_df: {e}")
                # falha de formatacao ncao deve quebrar a GUI; segue sem formatar
                pass
        else:
            # Usa verscao formatada do cache
        ...
        for row_idx in range(len(display_df)):
            ...
            for col_idx, col_name in enumerate(columns_list):
                ...
                if col_name not in [
                    ...
                    # Trunca apenas colunas que ncao scao de descricao
        ...
        try:
            ...
            logger.debug(f"Erro viewport width: {e}")
            vw = -1
        need_cols = (not hasattr(self, "_widths_columns_sig")) or (
            self._widths_columns_sig != cols_sig
        )
        need_vw = (not hasattr(self, "_last_viewport_w")) or (
            abs(vw - self._last_viewport_w) > 12
        )
        ...
        for i, col_name in enumerate(display_df.columns):
            ...
            # Se ncao ha largura calculada, usa configuracao salva manualmente pelo usuario
            if px is None:
            ...
            if px is None:
                if col_key == "#":
                    ...
                    px = 280  # Menor que descricao_ssa
                else:
            ...
            px = max(
                30, min(int(px), 1000)
            )  # Permite larguras maiores para descriptions
        ...
        try:
            ...
            logger.error(f"Erro ao restaurar modo interativo header: {e}")
        ...
        try:
            ...
            logger.error(f"Erro ao agendar _ensure_nonzero_column_widths: {e}")
    ...
    def display_data(self, df):  # usado em testes legados
        try:
            ...
            logger.error(f"Erro em display_data (legacy): {e}")
    ...
    def _force_column_widths(self):
        ...
        for i, col_name in enumerate(["#"] + self.visible_columns):
            # Busca largura salva das configuracões
            px = self._saved_gui_column_widths.get(col_name)
            if px is not None:
    ...
    def _ensure_nonzero_column_widths(self):
        """Garante que nenhuma coluna permaneca com largura 0.
        ...
        try:
            ...
            for i in range(col_count):
                if self.table_widget.columnWidth(i) == 0:
                    # Primero tenta dimensionar pelo conte├║do
                    try:
                        self.table_widget.resizeColumnToContents(i)
                    except Exception as e:
                        logger.debug(
                            f"Falha ao redimensionar coluna {i} pelo conteudo: {e}"
                        )
                    if self.table_widget.columnWidth(i) == 0:
    ...
    def _set_safe_width_for_col_index(self, idx: int, px: int = 80):
        ...
        try:
            ...
            logger.error(f"Erro _set_safe_width_for_col_index: {e}")
        ...
        try:
            ...
            if hasattr(df, "columns"):
                existing_visible_cols = [
                    col for col in self.visible_columns if col in df.columns
                ]
                if not existing_visible_cols:
                    logger.warning(
                        "ERRO: Nenhuma coluna visivel encontrada no DataFrame"
                    )
                    return

                # IMPORTANTE: Mantêm a ordem exata de self.visible_columns
                visible_df = df[existing_visible_cols].reindex(
                    columns=existing_visible_cols
                )
            else:
                visible_df = df
            ...
            if widget_width < 500:  # Tabela ainda ncao inicializada
                table_width = max(
                    1000 if sys.platform == "darwin" else 1400, self.width() - 50
                )
            else:
            ...
            # IMPORTANTE: Forca ordem correta das colunas (adiciona '#' no inácio)
            ...
            logger.error(f"ERRO em _compute_gui_column_widths: {e}")
            # Fallback para larguras mánimas das colunas visáveis apenas
            visible_cols = ["#"] + (
                self.visible_columns if hasattr(self, "visible_columns") else []
            )
    ...
    def _calculate_max_chars_for_column(self, col_name: str, col_idx: int) -> int:
        ...
        try:
            ...
            max_chars = max(15, int((width_px - 10) / 6.5))  # Melhores proporcões

            # Limites especáficos por tipo de coluna
            if col_name in ["descricao_ssa", "descricao_execucao"]:
                # Descricões podem usar toda largura disponável
                max_chars = max(50, max_chars)  # Mánimo mais alto para descricões
            elif col_name in ["numero_ssa", "localizacao_codigo"]:
                # Campos curtos ncao precisam de muito espaco
            ...
            logger.error(f"Erro em _calculate_max_chars_for_column: {e}")
    ...
    def _on_header_section_resized(
        ...
        try:
            ...
            logger.warning(f"Erro salvando redimensionamento coluna: {e}")
    ...
    def initiate_filtering(self):
        ...
        try:
            ...
            if search_text:
                ...
                if search_columns:
                    combined = pd.Series(
                        [" ".join(vals) for vals in zip(*search_columns)],
                        index=df.index,
                    )
                    ...
                    for term in terms:
                        if term.startswith("!"):
                            ...
                            if neg_term:
                                term_mask = ~combined.str.contains(
                                    neg_term, case=False, na=False, regex=False
                                )
                    ...
                    mask &= search_mask
                    logger.info(
                        f"Busca geral: {search_mask.sum()} registros encontrados"
                    )
            ...
            if self._active_column_filters:
                logger.info(
                    f"Aplicando {len(self._active_column_filters)} filtros de coluna"
                )
                for col_key, filter_term in self._active_column_filters.items():
                    ...
                    mask &= col_mask
                    logger.info(
                        f"Filtro coluna '{col_key}': {col_mask.sum()} registros"
                    )

            # 3. CHECKBOX "Nao está em STE/SCA"
            if hasattr(self, "_current_tab_index") and hasattr(self, "_tab_contexts"):
                if self._current_tab_index >= 0 and self._current_tab_index < len(
                    self._tab_contexts
                ):
                    ctx = self._tab_contexts[self._current_tab_index]
                    ...
                    if (
                        exclude_ste
                        and _is_widget_valid(exclude_ste)
                        and exclude_ste.isChecked()
                    ):
                        logger.info("Aplicando exclusao STE/SCA")
                        if "situacao" in df.columns:
                            ste_mask = ~df["situacao"].astype(str).str.upper().isin(
                                ["STE", "SCA"]
                            )
                            mask &= ste_mask
    ...
    def _format_details_html(
        self, series, highlight_search_terms=False, font_size_pt=None, linkify=False
    ):
        ...
        search_terms = self._collect_highlight_terms() if highlight_search_terms else []
        ...
        try:
            ...
            logger.warning(f"Erro ao obter cores do tema para HTML: {e}")
            text_color = "#000000"
            link_color = text_color

        html_lines = [
            f'<html><body style="font-family: monospace; font-size: {font_size_pt}pt; color: {text_color};">'
        ]
        ...
        for col, value in sorted_items:
            ...
            display_name = DETAIL_DISPLAY_OVERRIDES.get(
                col, self.internal_to_display.get(col, col)
            )
        ...
        try:
            ...
            logger.error(f"Erro ao obter derivadas para HTML: {e}")
            derived_list = []
        if derived_list:
            if linkify:
                items = []
                for item in derived_list:
                    href = normalize_ssa_value(item)
    ...
    def on_table_double_click(self, index):
        ...
        - Aba SSAs: Abre dialog de resumo rápido com opcao de abrir em Detalhes
        ...
        if tab_kind == "filters":
            if numero_ssa:
                ...
                logger.warning(
                    "POPUP: Abrindo QMessageBox.information - Numero SSA nao encontrado"
                )
        ...
        html_content = self._format_details_html(
            series, highlight_search_terms=True, linkify=True
        )
        ...
        if numero_ssa:
            ...
            open_tab_button.clicked.connect(
                lambda: self._open_ssa_in_details_tab(numero_ssa)
            )
    ...
    def _save_page_size_pref(self, new_size: int):
        """Persiste o tamanho da pagina no settings."""
        # Nota: Persistencia removida para isolamento do CLI
        # O tamanho da pagina fica configurado no arquivo gui_main_preferences.json
        pass
    ...
    def _get_series_from_row(self, row: int):
        try:
            ...
            logger.error(f"Erro ao acessar item da linha {row}: {e}")
            return None
        if not index_item:
            return None
        try:
            ...
            logger.error(f"Erro ao ler UserRole da linha {row}: {e}")
            original_index = None
        if original_index is None or not (0 <= original_index < len(self.df_exibido)):
            return None
        try:
            ...
            logger.error(f"Erro ao acessar df_exibido em iloc[{original_index}]: {e}")
            return None
        ...
        try:
            ...
            if hasattr(self, "details_group"):
                try:
                    ...
                    logger.debug(f"Erro ao calcular tamanho da fonte detalhes: {e}")
                    font_size_pt = None
            html_content = self._format_details_html(
                series,
            ...
            logger.error(f"Erro ao atualizar detalhes em HTML: {e}")
            pass
    ...
    def _get_derivadas_for_ssa(self, numero_ssa):
        if self.df_completo is None or self.df_completo.empty:
            return []
        if (
            "derivada_de" not in self.df_completo.columns
            or "numero_ssa" not in self.df_completo.columns
        ):
            return []
        ...
        try:
            ...
            for value in derived_raw:
                ...
                if formatted:
                    derived_list.append(formatted)
            return derived_list
        except Exception as e:
            logger.error(f"Erro ao processar derivadas: {e}")
            return []
    ...
    def _jump_to_ssa(self, numero_ssa):
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Erro ao setar current_page no paginator: {e}")
            self.display_current_page(page)
            row_in_page = int(pos % page_size)
            try:
                self.table_widget.selectRow(row_in_page)
            except Exception as e:
                logger.debug(f"Erro ao selecionar linha {row_in_page} apos salto: {e}")
        except Exception as e:
            logger.error(f"Erro em _jump_to_ssa: {e}")
            pass
    ...
    def _on_details_anchor_clicked(self, url):
        try:
            ...
            logger.warning(f"Erro ao processar URL clicada: {e}")
            return
        if not href:
            return
    ...
    def _open_ssa_in_details_tab(self, numero_ssa):
        ...
        try:
            # Muda para aba Detalhes (busca dinamica de indice)
            idx = -1
            if hasattr(self, "tab_details"):
                idx = self.main_tabs.indexOf(self.tab_details)
            ...
            if idx >= 0:
                ...
                logger.warning(
                    "Aba Detalhes nao encontrada via referencia, tentando indice fixo 2"
                )
    ...
    def _filter_by_derivadas(self, numero_ssa):
        ...
        try:
            ...
            logger.error(f"Erro ao reconstruir painel de filtros (derivadas): {e}")
        self._refresh_after_filter_change()
    ...
    def _clear_derivadas_filter(self):
        ...
        try:
            ...
            logger.error(f"Erro ao reconstruir painel de filtros (clear): {e}")
        self._refresh_after_filter_change()
        ...
        if self.table_widget.itemAt(position):
            ...
            if current_item:
                try:
                    ...
                    logger.debug(
                        f"Nao foi possivel obter dados da linha para context menu: {e}"
                    )
                    row_series = None
            ...
            if row_series is not None:
                ...
                if numero_ssa:
                    ...
                    detalhes_action.triggered.connect(
                        lambda: self._open_ssa_in_details_tab(numero_ssa)
                    )
                ...
                if derivada_de:
                    origem_action = QAction("Ir para SSA de origem", self)
                ...
                if self._last_derivada_origem:
                    voltar_action = QAction("Voltar para SSA de origem", self)
            ...
            if current_item:
                ...
                if column > 0:  # Ncao permitir remover a coluna de ándice
    ...
    def _export_current_list_txt(self):
        ...
        try:
            ...
            logger.warning(f"Erro no diálogo de salvar arquivo: {e}")
            path = ""
        if not path:
            return
    ...
    def _update_filters_summary(self):
        ...
        try:
            ...
            logger.info(
                f"hasattr _current_tab_index: {hasattr(self, '_current_tab_index')}"
            )
            logger.info(f"hasattr _tab_contexts: {hasattr(self, '_tab_contexts')}")

            if not hasattr(self, "_current_tab_index") or not hasattr(
                self, "_tab_contexts"
            ):
            ...
            if self._current_tab_index < 0 or self._current_tab_index >= len(
                self._tab_contexts
            ):
                logger.error(
                    f"ERRO: _current_tab_index {self._current_tab_index} fora do range [0, {len(self._tab_contexts)})"
                )
                return

            ctx = self._tab_contexts[self._current_tab_index]
            logger.info(
                f"Contexto da aba {self._current_tab_index}: {list(ctx.keys())}"
            )
            ...
            if not label:
                logger.error(
                    f"ERRO: label nao encontrado no contexto! Keys disponiveis: {list(ctx.keys())}"
                )
                return
            ...
            logger.info(
                f"_active_column_filters COMPLETO: {dict(self._active_column_filters)}"
            )
            logger.info(f"display_map tem {len(self.display_map)} entradas")
            logger.info(
                f"Primeiras 10 keys do display_map: {list(self.display_map.keys())[:10]}"
            )

            if self._active_column_filters:
                logger.info(
                    f"Iterando sobre {len(self._active_column_filters)} filtros:"
                )
                for idx, (col, term) in enumerate(
                    self._active_column_filters.items(), 1
                ):
                    logger.info(f"  [{idx}] col='{col}' (tipo: {type(col).__name__})")
                    logger.info(
                        f"  [{idx}] term='{term}' (tipo: {type(term).__name__}, len: {len(term) if term else 0})"
                    )
                    ...
                    logger.info(
                        f"  [{idx}] active_filters agora tem {len(active_filters)} itens"
                    )
            else:
                logger.info("_active_column_filters esta VAZIO")

            # Checkbox "Nao está em STE/SCA"
            exclude_ste = ctx.get("exclude_ste_checkbox")
            ...
            if self._current_tab_index == 1:
                ...
                emis_selected = [
                    self._checkbox_value(cb)
                    for cb in emis_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                emis_excluded = [
                    self._checkbox_value(cb)
                    for cb in emis_exclude
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if emis_selected:
                ...
                if emis_excluded:
                    active_filters.append(
                        f"Emissor (excluir): {', '.join(emis_excluded)}"
                    )
                ...
                exec_selected = [
                    self._checkbox_value(cb)
                    for cb in exec_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                exec_excluded = [
                    self._checkbox_value(cb)
                    for cb in exec_exclude
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if exec_selected:
                ...
                if exec_excluded:
                    active_filters.append(
                        f"Executor (excluir): {', '.join(exec_excluded)}"
                    )
                ...
                div_selected = [
                    self._checkbox_value(cb)
                    for cb in div_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                div_excluded = [
                    self._checkbox_value(cb)
                    for cb in div_exclude
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if div_selected:
                ...
                if div_excluded:
                    active_filters.append(
                        f"Divisao (excluir): {', '.join(div_excluded)}"
                    )
                ...
                if status_excluded:
                    active_filters.append(
                        f"Situacao (excluir): {', '.join(status_excluded)}"
                    )
                ...
                year_emis_selected = [
                    self._checkbox_value(cb)
                    for cb in year_emis_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if year_emis_selected:
                    active_filters.append(
                        f"Ano Emissao: {', '.join(year_emis_selected)}"
                    )
                ...
                year_exec_selected = [
                    self._checkbox_value(cb)
                    for cb in year_exec_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if year_exec_selected:
                    active_filters.append(
                        f"Ano Execucao: {', '.join(year_exec_selected)}"
                    )
                ...
                reprog_selected = [
                    self._checkbox_value(cb)
                    for cb in reprog_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if (
                    reprog_selected
                    and reprog_mode_widget
                    and _is_widget_valid(reprog_mode_widget)
                ):
                    mode_text = reprog_mode_widget.currentText()
                    active_filters.append(
                        f"Reprogramacoes {mode_text}: {', '.join(reprog_selected)}"
                    )
                ...
                prio_emis_selected = [
                    self._checkbox_value(cb)
                    for cb in prio_emis_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if prio_emis_selected:
                    active_filters.append(
                        f"Prio. Emissao: {', '.join(prio_emis_selected)}"
                    )

                # 9. MULTISELECT: Prio. Planejamento
                prio_plan_checks = getattr(
                    self, "adv_prioridade_planejamento_checks", []
                )
                prio_plan_selected = [
                    self._checkbox_value(cb)
                    for cb in prio_plan_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if prio_plan_selected:
                    active_filters.append(
                        f"Prio. Planejamento: {', '.join(prio_plan_selected)}"
                    )

                # 10. MULTISELECT: Solicitante
                sol_checks = getattr(self, "adv_responsavel_solicitante_checks", [])
                sol_exclude = getattr(
                    self, "adv_responsavel_solicitante_exclude_checks", []
                )
                sol_selected = [
                    self._checkbox_value(cb)
                    for cb in sol_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                sol_excluded = [
                    self._checkbox_value(cb)
                    for cb in sol_exclude
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if sol_selected:
                ...
                if sol_excluded:
                    active_filters.append(
                        f"Solicitante (excluir): {', '.join(sol_excluded)}"
                    )

                # 11. MULTISELECT: Resp Prog
                prog_checks = getattr(self, "adv_responsavel_programacao_checks", [])
                prog_exclude = getattr(
                    self, "adv_responsavel_programacao_exclude_checks", []
                )
                prog_selected = [
                    self._checkbox_value(cb)
                    for cb in prog_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                prog_excluded = [
                    self._checkbox_value(cb)
                    for cb in prog_exclude
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                if prog_selected:
                ...
                if prog_excluded:
                    active_filters.append(
                        f"Resp Prog (excluir): {', '.join(prog_excluded)}"
                    )

                # 12. MULTISELECT: Resp Exec
                exec_resp_checks = getattr(self, "adv_responsavel_execucao_checks", [])
                exec_resp_exclude = getattr(
                    self, "adv_responsavel_execucao_exclude_checks", []
                )
                exec_resp_selected = [
                    self._checkbox_value(cb)
                    for cb in exec_resp_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                exec_resp_excluded = [
                    self._checkbox_value(cb)
                    for cb in exec_resp_exclude
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                ...
                if exec_resp_excluded:
                    active_filters.append(
                        f"Resp Exec (excluir): {', '.join(exec_resp_excluded)}"
                    )

                # 13. MULTISELECT: Resp Emis
                emis_resp_checks = getattr(self, "adv_responsavel_emissor_checks", [])
                emis_resp_exclude = getattr(
                    self, "adv_responsavel_emissor_exclude_checks", []
                )
                emis_resp_selected = [
                    self._checkbox_value(cb)
                    for cb in emis_resp_checks
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                emis_resp_excluded = [
                    self._checkbox_value(cb)
                    for cb in emis_resp_exclude
                    if _is_widget_valid(cb) and cb.isChecked()
                ]
                ...
                if emis_resp_excluded:
                    active_filters.append(
                        f"Resp Emis (excluir): {', '.join(emis_resp_excluded)}"
                    )
                ...
                if (
                    week_emis_start
                    and _is_widget_valid(week_emis_start)
                    and week_emis_start.text().strip()
                ):
                    if (
                        week_emis_end
                        and _is_widget_valid(week_emis_end)
                        and week_emis_end.text().strip()
                    ):
                        active_filters.append(
                            f"Emissao AnoSemana: {week_emis_start.text().strip()} - {week_emis_end.text().strip()}"
                        )
                    else:
                        active_filters.append(
                            f"Emissao AnoSemana: >= {week_emis_start.text().strip()}"
                        )
                elif (
                    week_emis_end
                    and _is_widget_valid(week_emis_end)
                    ...
                    active_filters.append(
                        f"Emissao AnoSemana: <= {week_emis_end.text().strip()}"
                    )
                ...
                if (
                    week_exec_start
                    and _is_widget_valid(week_exec_start)
                    and week_exec_start.text().strip()
                ):
                    if (
                        week_exec_end
                        and _is_widget_valid(week_exec_end)
                        and week_exec_end.text().strip()
                    ):
                        active_filters.append(
                            f"Execucao AnoSemana: {week_exec_start.text().strip()} - {week_exec_end.text().strip()}"
                        )
                    else:
                        active_filters.append(
                            f"Execucao AnoSemana: >= {week_exec_start.text().strip()}"
                        )
                elif (
                    week_exec_end
                    and _is_widget_valid(week_exec_end)
                    ...
                    active_filters.append(
                        f"Execucao AnoSemana: <= {week_exec_end.text().strip()}"
                    )
                ...
                if (
                    macro_combo
                    and _is_widget_valid(macro_combo)
                    and macro_combo.currentIndex() > 0
                ):
                    macro_text = macro_combo.currentText()
                    active_filters.append(f"Macro: {macro_text}")

                logger.info(
                    f"Filtros avancados adicionados: {len(active_filters)} total"
                )
            ...
            if active_filters:
                summary_text = (
                    f"{len(active_filters)} filtro(s): {' | '.join(active_filters)}"
                )
                logger.info(f"========================================")
                logger.info(f"TEXTO FINAL A SER EXIBIDO:")
    ...
    def _update_col_filter_indicator(self):
        ...
        try:
            ...
            if self._current_tab_index < 0 or self._current_tab_index >= len(
                ...
                logger.warning(
                    f"col_filter_indicator: _current_tab_index {self._current_tab_index} invalido"
                )
                return
            ctx = self._tab_contexts[self._current_tab_index]
            ...
            # UIREFACTOR 2026-01-14: Ocultar indicador na aba "Filtros" (indice 1)
            if self._current_tab_index == 1:
            ...
            # LAZY LOADING: Nao mostrar indicador se dados completos ainda nao carregaram
            # Evita mostrar valores temporários/incorretos durante inicializacao
            if not getattr(self, "_is_full_data_loaded", False):
                logger.debug(
                    "col_filter_indicator: OCULTO - dados completos ainda nao carregados"
                )
                indicator.setText("")
                indicator.setVisible(False)
                return
    ...
    def _clear_all_filters_global(self):
        ...
        try:
            ...
            # CORRECAO LAYOUTREFACTOR #2: Limpar filtros avancados e derivadas
    ...
    def _update_undo_button_state(self):
        ...
        try:
            ...
            if undo_btn:
                # FASE6: Habilitar botão quando houver histórico de filtros
                has_history = (
                    hasattr(self, "_filter_history") and len(self._filter_history) > 0
                )
                undo_btn.setEnabled(has_history)
        except Exception as e:
            logger.error(f"Erro ao atualizar estado do botao undo: {e}")
    ...
    def _apply_permanent_filter(self, column_key, line_edit):
        ...
        try:
            ...
            logger.info(f"############################################################")
            logger.info(
                f"Coluna KEY: '{column_key}' (tipo: {type(column_key).__name__})"
            )
            logger.info(
                f"Termo DIGITADO: '{term}' (tipo: {type(term).__name__}, len: {len(term)})"
            )
            ...
            logger.error(
                f"ERRO CRITICO ao aplicar filtro permanente para {column_key}: {e}",
                exc_info=True,
            )
    ...
    def _remove_permanent_filter(self, column_key, line_edit):
        ...
        try:
            ...
            logger.error(
                f"Erro ao remover filtro permanente para {column_key}: {e}",
                exc_info=True,
            )
    ...
    def _open_add_column_filter_menu(self):
        ...
        all_cols = sorted(
            [col for col in self.display_map.keys() if col not in permanent_cols]
        )
        ...
        if ok and col_display:
            ...
            term, ok = QInputDialog.getText(
                self, "Filtro", f"Termo para filtrar '{col_display}':"
            )
    ...
    def _build_column_filters_panel(self):
        """Reconstroi o painel de filtros de coluna (permanentes + adicionais na área de scroll)."""
        try:
            logger.info(">>> _build_column_filters_panel INICIADO")
            logger.info(
                f"self._current_tab_index = {getattr(self, '_current_tab_index', 'NAO EXISTE')}"
            )
            logger.info(
                f"len(self._tab_contexts) = {len(getattr(self, '_tab_contexts', []))}"
            )
            ...
            if self._current_tab_index < 0 or self._current_tab_index >= len(
                ...
                logger.warning(
                    f"_build_column_filters_panel: tab_index {self._current_tab_index} invalido"
                )
                return

            ctx = self._tab_contexts[self._current_tab_index]
            logger.info(
                f"Contexto obtido: tab_kind={ctx.get('tab_kind')}, id={id(ctx)}"
            )
            logger.info(f"Keys disponiveis: {sorted(ctx.keys())}")

            permanent_inputs = ctx.get("permanent_filter_inputs", {})
            logger.info(
                f"permanent_filter_inputs encontrado: {len(permanent_inputs)} inputs"
            )
            ...
            for col_key, line_edit in permanent_inputs.items():
                ...
                if line_edit.text() != current_filter:
                    ...
                    logger.info(
                        f"  Permanente '{col_key}' atualizado para: '{current_filter}'"
                    )

            logger.info("_build_column_filters_panel CONCLUIDO")
        except Exception as e:
            logger.error(
                f"Erro ao reconstruir painel de filtros de coluna: {e}", exc_info=True
            )
    ...
    def remove_column_by_index(self, column_index):
        ...
        if column_index > 0 and column_index < len(
            self.visible_columns
        ):  # Protege coluna de ándice
            internal_column = self.visible_columns[
                column_index - 1
            ]  # -1 porque ha coluna '#'
...
<p>Use a aba Filtros para combinacoes complexas de criterios.</p>
    ...
    def auto_fit_column(self, column_index):
        ...
        if header_item and column_index > 0:  # Ncao salvar largura da coluna de ándice
            internal_column = (
                self.visible_columns[column_index - 1]
                if column_index <= len(self.visible_columns)
                else None
            )
            if internal_column:
    ...
    def rescan_data(self):
        ...
        if not os.path.exists(main_py_path):
            QMessageBox.warning(
                self, "Erro", f"Arquivo main.py nao encontrado em {main_py_path}"
            )
            return
            ...
            try:
                ...
                if test_df is not None and not test_df.empty:
                    ...
                    QMessageBox.warning(
                        self,
                        "Erro",
                        "O arquivo selecionado nao contem dados validos na tabela 'ssas'.",
                    )
            except Exception as e:
    ...
    def resizeEvent(self, event):
        ...
        if (
            ...
            if width_change > 12:  # So recalcula se mudanca for > 12px
                # Delay para evitar recalculos excessivos durante resize
    ...
    def _recompute_column_widths_on_resize(self):
        ...
        try:
            # Verifica se widgets estcao em estado valido
            if (
            ...
            # Recalcula larguras com nova dimenscao da janela usando WidthManager
    ...
    def _apply_computed_widths_only(self):
        """Aplica apenas as larguras calculadas pelo WidthManager (ignora configuracões salvas)."""
        try:
            if (
                not hasattr(self, "df_para_tabela")
                or self.df_para_tabela.empty
    ...
    def closeEvent(self, event):
        ...
        if (
            hasattr(self, "data_loader_thread")
            and self.data_loader_thread
            and self.data_loader_thread.isRunning()
        ):
