...
def _build_advanced_filters_panel(self):
    ...
    reprog_button = QToolButton()
    reprog_button.setText("Selecionar")
    reprog_menu = QMenu(reprog_button)
    self._attach_multiselect_menu(reprog_button, reprog_menu)
    reprog_button.setToolTip("Selecionar")
    try:
    ...
    main_grid.addWidget(div_box, 0, 0)
    main_grid.addWidget(status_box, 0, 1)
    main_grid.addWidget(year_emissao_box, 0, 2)
    main_grid.addWidget(year_execucao_box, 0, 3)
    ...
    main_grid.addWidget(deriv_box, 2, 2, 1, 2)
    main_grid.addWidget(sol_box, 3, 0)
    main_grid.addWidget(prog_box, 3, 1)
    main_grid.addWidget(exec_resp_box, 3, 2, 1, 2)
    for col in range(4):
...
def _show_derivadas_popup(self):
    """Mantido por compatibilidade de facade. Popup removido por decisao de UX."""
    return

def _update_derivadas_button_state(self):
    """Mantido por compatibilidade de facade. Sem estado visual para atualizar."""
    return
...
def _reorganize_advanced_filters_grid(self, width: int):
    ...
    layouts = {
        "wide": [
            ("div_box", 0, 0, 1, 1),
            ("status_box", 0, 1, 1, 1),
            ("year_emissao_box", 0, 2, 1, 1),
            ("year_execucao_box", 0, 3, 1, 1),
            ("reprog_box", 1, 0, 1, 1),
            ("prio_emis_box", 1, 1, 1, 1),
            ("prio_plan_box", 1, 2, 1, 1),
            ("macro_box", 1, 3, 1, 1),
            ("week_emis_box", 2, 0, 1, 1),
            ("week_exec_box", 2, 1, 1, 1),
            ("deriv_box", 2, 2, 1, 2),
            ("sol_box", 3, 0, 1, 1),
            ("prog_box", 3, 1, 1, 1),
            ("exec_resp_box", 3, 2, 1, 2),
        ],
        "mid": [
            ("div_box", 0, 0, 1, 1),
            ("status_box", 0, 1, 1, 1),
            ("year_emissao_box", 0, 2, 1, 1),
            ("year_execucao_box", 1, 0, 1, 1),
            ("reprog_box", 1, 1, 1, 1),
            ("prio_emis_box", 1, 2, 1, 1),
            ("prio_plan_box", 2, 0, 1, 1),
            ("macro_box", 2, 1, 1, 1),
            ("deriv_box", 2, 2, 1, 1),
            ("week_emis_box", 3, 0, 1, 1),
            ("week_exec_box", 3, 1, 1, 1),
            ("sol_box", 3, 2, 1, 1),
            ("prog_box", 4, 0, 1, 1),
            ("exec_resp_box", 4, 1, 1, 2),
        ],
        "narrow": [
            ("div_box", 0, 0, 1, 1),
            ("status_box", 0, 1, 1, 1),
            ("year_emissao_box", 1, 0, 1, 1),
            ("year_execucao_box", 1, 1, 1, 1),
            ("reprog_box", 2, 0, 1, 1),
            ("prio_emis_box", 2, 1, 1, 1),
            ("prio_plan_box", 3, 0, 1, 1),
            ("macro_box", 3, 1, 1, 1),
            ("week_emis_box", 4, 0, 1, 1),
            ("week_exec_box", 4, 1, 1, 1),
            ("deriv_box", 5, 0, 1, 2),
            ("sol_box", 6, 0, 1, 1),
            ("prog_box", 6, 1, 1, 1),
            ("exec_resp_box", 7, 0, 1, 2),
        ],
    }
    columns_by_mode = {"wide": 4, "mid": 3, "narrow": 2}
    required_keys = {entry[0] for entries in layouts.values() for entry in entries}
    missing = [key for key in required_keys if key not in w]
    if missing:
        logger.warning("Falha ao reorganizar filtros avancados. Widgets ausentes: %s", ", ".join(sorted(missing)))
        return
    ...
    for widget_key, row, col, row_span, col_span in layouts[mode]:
        widget = w.get(widget_key)
        if not _is_widget_valid(widget):
            logger.warning("Widget invalido ignorado no reorganize de filtros: %s", widget_key)
            continue
        grid.addWidget(widget, row, col, row_span, col_span)
    for col in range(columns_by_mode[mode]):
...
def _apply_divisao_to_setor_checks(self):
    """Compat hook mantido para facade; filtros de setor foram removidos da UI."""
    return
...
def _refresh_responsavel_options(self, target_prefixes=None):
    ...
    has_sector = bool(div_values or div_excluded)
    def apply_cb():
    ...
    def _apply_sector_subset(frame):
        ...
        if exec_col in subset.columns:
            allowed = set(div_setores)
            excluded = set(div_setores_excluded)
            if allowed:
...
def _collect_advanced_filters_from_ui(self) -> dict:
    data = {}
    try:
...
def _apply_advanced_filters_from_ui(self, store_only: bool = False):
    ...
    data = self._collect_advanced_filters_from_ui()
...
def _populate_advanced_filter_values_cache(self, cache: dict, df: pd.DataFrame) -> dict:
    def _unique_sorted(col):
        try:
            vals = df[col].dropna().astype(str).str.strip()
            vals = vals[vals != ""]
...
def _refresh_advanced_filter_options(self):
    ...
    try:
        ...
        cache = self._populate_advanced_filter_values_cache(cache, df)
        ...
        self._refresh_sector_menus(divisao_vals, status_vals, filters, apply_cb)
