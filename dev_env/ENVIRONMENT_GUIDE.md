...
Este documento complementa o `README.md` do diret?rio `dev_env` e descreve em detalhe como o fluxo unificado de ambiente funciona nas diferentes plataformas.
...
- `scripts/env/direnv_common.sh`: n?cleo compartilhado. Resolve a variante (`stable` ou `free-threaded`), provisiona pyenv ou `.venv` e exporta PATH/vari?veis.
...
2. `ssa_env::apply` determina a variante desejada seguindo a prioridade:
   ...
   2. vari?veis `SSA_PYTHON_VARIANT` ou `SSA_USE_FREE_THREADED`;
...
4. Se pyenv n?o estiver dispon?vel:
...
6. Um log `[env] python ...` informa a vers?o e a origem (`pyenv-virtualenv`, `pyenv`, `venv`).

## 3. Variantes dispon?veis
- **stable** (padr?o): vers?o definida em `.python-version` (hoje `3.13.7`).
...
- `pyenv version` / `pyenv virtualenvs`: lista vers?es dispon?veis.
- `echo $SSA_ENV_SOURCE` (bash) ou `$env:SSA_ENV_SOURCE` (PowerShell): mostra investimento atual.
- `pyenv install --list | grep 3.14`: verifica se a build `3.14-dev` est? dispon?vel no mirror local.

## 5. Boas pr?ticas
- Sempre rodar `direnv allow` ap?s alterar `.envrc` ou `scripts/env/direnv_common.sh`.
- Em Windows, considere adicionar `. .\activate_repo.ps1` ao perfil para ativa??o autom?tica.
- Use `SSA_AUTO_INSTALL_REQ=1` caso queira for?ar `pip install -r requirements.txt` no fallback `.venv`.
- Ao testar builds free-threaded, valide se alguma depend?ncia nativa suporta o bin?rio `--disable-gil`.
...
3. Se necess?rio, exclua `.venv` ou `.venv_ft` e repita o passo anterior.

Com isso, o setup permanece previs?vel em Windows, WSL Debian e macOS.
