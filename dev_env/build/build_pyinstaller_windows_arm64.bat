...
if "%SILENT%"=="1" (
    uv run --python "%UV_PYTHON%" "%REPO_ROOT%\launchers\build_multiplatform.py" --platform windows_arm64 --clean >nul 2>&1
) else (
    echo Limpando artefatos PyInstaller anteriores...
    uv run --python "%UV_PYTHON%" "%REPO_ROOT%\launchers\build_multiplatform.py" --platform windows_arm64 --clean
)
...
if "%SILENT%"=="1" (
    ...
    uv run --python "%UV_PYTHON%" "%REPO_ROOT%\launchers\build_multiplatform.py" --platform windows_arm64 --apps cli gui %RUNTIME_DB_ARGS% > "%LOG_FILE%" 2>&1
) else (
    echo Iniciando build PyInstaller windows_arm64...
    uv run --python "%UV_PYTHON%" "%REPO_ROOT%\launchers\build_multiplatform.py" --platform windows_arm64 --apps cli gui %RUNTIME_DB_ARGS%
)
...
if "%SILENT%"=="1" (
    uv run --python "%UV_PYTHON%" "%REPO_ROOT%\scripts\sync_pyinstaller_outputs.py" --platform windows_arm64 --quiet >> "%LOG_FILE%" 2>&1
) else (
    uv run --python "%UV_PYTHON%" "%REPO_ROOT%\scripts\sync_pyinstaller_outputs.py" --platform windows_arm64
)
