...
if "%SILENT%"=="1" (
    uv run --frozen --python "%UV_PYTHON%" "%REPO_ROOT%\launchers\build_multiplatform.py" --platform windows_amd64 --clean >nul 2>&1
) else (
    echo Limpando artefatos PyInstaller anteriores...
    uv run --frozen --python "%UV_PYTHON%" "%REPO_ROOT%\launchers\build_multiplatform.py" --platform windows_amd64 --clean
)
...
if "%SILENT%"=="1" (
    ...
    uv run --frozen --python "%UV_PYTHON%" "%REPO_ROOT%\launchers\build_multiplatform.py" --platform windows_amd64 --apps cli gui %RUNTIME_DB_ARGS% > "%LOG_FILE%" 2>&1
) else (
    echo Iniciando build PyInstaller windows_amd64...
    uv run --frozen --python "%UV_PYTHON%" "%REPO_ROOT%\launchers\build_multiplatform.py" --platform windows_amd64 --apps cli gui %RUNTIME_DB_ARGS%
)
...
if "%SILENT%"=="1" (
    uv run --frozen --python "%UV_PYTHON%" "%REPO_ROOT%\scripts\sync_pyinstaller_outputs.py" --platform windows_amd64 --quiet >> "%LOG_FILE%" 2>&1
) else (
    uv run --frozen --python "%UV_PYTHON%" "%REPO_ROOT%\scripts\sync_pyinstaller_outputs.py" --platform windows_amd64
)
...
if "%SILENT%"=="0" (
    ...
    if /I "!DO_CLEANUP!"=="s" (
        uv run --frozen --python "%UV_PYTHON%" "%REPO_ROOT%\scripts\cleanup_build_artifacts.py" --scope temp
    )
)
if "%SILENT%"=="0" pause
exit /b 0
