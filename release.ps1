...
function Show-Usage {
    ...
    Write-Host "  Instalador Windows: ativado por padrao no AMD64; ARM64 exige -SkipInstaller (ZIP)"
    Write-Host ""
...
function Invoke-WindowsRelease {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Release Windows falhou: codigo=$LASTEXITCODE; alvo=$TargetPlatform; backend=$BackendCsv; script=$script."
        }
    } finally {
